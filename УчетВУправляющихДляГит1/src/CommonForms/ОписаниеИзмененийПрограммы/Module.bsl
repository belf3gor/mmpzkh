...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДокументОписаниеОбновлений.ВысотаТаблицы = 0 Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Конфигурация успешно обновлена на версию %1'"), КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия));
		// Квартплата -
	...
	СтруктрураВерсийПриОбновлении = УПЖКХ_ТиповыеМетодыВызовСервера.ХранилищеОбщихНастроекЗагрузить("СтруктрураВерсийПриОбновлении","",,,ТекущийПользователь);
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		Если СтруктрураВерсийПриОбновлении <> Неопределено И Параметры.ПоказыватьТолькоИзменения Тогда
			ДокументОписаниеОбновленийЖКХ = УПЖКХ_ОбновлениеИнформационнойБазы.ДокументОписаниеОбновлений(СтруктрураВерсийПриОбновлении);
		Иначе
			ДокументОписаниеОбновленийЖКХ = ПолучитьОбщийМакет(МакетОписаниеОбновлений);
		КонецЕсли;
	Иначе
		ДокументОписаниеОбновленийЖКХ = Новый ТабличныйДокумент();
	КонецЕсли;
	
	Если СтруктрураВерсийПриОбновлении <> Неопределено Тогда
		УПЖКХ_ТиповыеМетодыВызовСервера.ХранилищеОбщихНастроекУдалить("СтруктрураВерсийПриОбновлении","",ТекущийПользователь);
	КонецЕсли;
	
	Если ДокументОписаниеОбновленийЖКХ.ВысотаТаблицы = 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Конфигурация успешно обновлена на версию %1'"), Метаданные.Версия);
		ДокументОписаниеОбновленийЖКХ.Область("R1C1:R1C1").Текст = Текст;
	КонецЕсли;
	
	ОписаниеОбновленийЖКХ.Очистить();
	ОписаниеОбновленийЖКХ.Вывести(ДокументОписаниеОбновленийЖКХ);
...
&НаКлиенте
// Обработчик события "ОбработкаНавигационнойСсылки" поля "АдресФормыВПрограмме".
//
Процедура АдресФормыВПрограммеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьОписаниеИзмененийБП" Тогда
