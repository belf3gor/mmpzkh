...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Квартплата +
	// Параметр используется в адресах прибытия и выбытия жильца.
	Если Параметры.Свойство("УПЖКХ_Период") Тогда
		
		ДобавляемыеРеквизиты = Новый Массив;
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("УПЖКХ_Период", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.Дата)), , "Период", Истина));
		ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		
		ПолеВводаПериода = Элементы.Добавить("УПЖКХ_Период", Тип("ПолеФормы"), Элементы.НациональныйАдрес);
		ПолеВводаПериода.ПутьКДанным = "УПЖКХ_Период";
		ПолеВводаПериода.Вид = ВидПоляФормы.ПолеВвода;
		
		ЭтаФорма.УПЖКХ_Период = Параметры.УПЖКХ_Период;
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда // При немодифицированности работает как "отмена".
		...
		// Квартплата +
		Если Не Элементы.Найти("УПЖКХ_Период") = Неопределено Тогда
			Результат.Вставить("УПЖКХ_Период", ЭтаФорма.УПЖКХ_Период);
		КонецЕсли;
		// Квартплата -
