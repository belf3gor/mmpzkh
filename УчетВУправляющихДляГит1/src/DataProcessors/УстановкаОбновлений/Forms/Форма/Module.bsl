...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	Если НоваяТекущаяСтраница = Страницы.ОбновленияНеОбнаружено Тогда
		...
		Элементы.НадписьОписаниеТекущейКонфигурации.Заголовок = ПараметрыИБ.СинонимКонфигурации;
		Элементы.НадписьВерсияТекущейКонфигурации.Заголовок   = ПараметрыИБ.КВП_НомерВерсииКВПВМетаданных;
		// Квартплата -
