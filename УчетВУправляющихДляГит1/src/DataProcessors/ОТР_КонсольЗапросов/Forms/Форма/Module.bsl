...
Процедура ЗаполнитьПараметрыИзXMLСервер(Знач СтрокаXML, Знач Контекст)
	...
	Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
		...
		Если ТипЗнч(Значение) = Тип("ТаблицаЗначений") Тогда
			ПараметрЗапроса.ЗначениеВФорме = ОбъектОбработки().ФормированиеПредставленияЗначения(Значение);
		Иначе
...
// Обработчик команды "ОТР_ПолучитьЗапрос" формы.
//
&НаКлиенте
Процедура ОТР_ПолучитьЗапрос(Команда)
	
	ОписаниеЗапроса = ПолучитьЗапросНаСервере();
	
	Если ОписаниеЗапроса = Неопределено Тогда
		
		ПоказатьПредупреждение(Неопределено, "Не найдено описание запроса.
			|Получить описание запроса можно в режиме отладки
			|с помощью вычисления выражения ""ОТР_КонсольЗапросов.ЗапросВКонсоль(Запрос)"".");
		
	Иначе
		
		ОчиститьПараметрыНаСервере();
		
		Контекст = Новый Структура("ИзТекстаЗапроса", Истина);
		
		ЗаполнитьПараметрыИзXMLСервер(ОписаниеЗапроса, Контекст);
...
&НаСервере
// Удаляет записи в табличной части "Параметры".
//
...
&НаСервере
// Получает описание запроса в формате XML из хранилища.
// 
...
Функция ПолучитьЗапросНаСервере()
	...
	Возврат ОТР_КонсольЗапросов.ПолучитьЗапрос();
	
КонецФункции

&НаСервере
// Удалить описание запроса из хранилища.
//
Процедура УдалитьЗапросНаСервере()
	
	ОТР_КонсольЗапросов.УдалитьЗапрос();
	
КонецПроцедуры

&НаКлиенте
// Обработчик события "ОбработкаНавигационнойСсылки" подсказки кнопки "ФормаОТР_ПолучитьЗапрос".
// Помещает в буфер обмена текст "ОТР_КонсольЗапросов.ЗапросВКонсоль(Запрос)".
//
Процедура ФормаОТР_ПолучитьЗапросРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "СкопироватьКомандуЗапросВКонсоль" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		СкопироватьТекстВБуферОбмена("ОТР_КонсольЗапросов.ЗапросВКонсоль(Запрос)");
		
		//ОписаниеОповещения = Новый ОписаниеОповещения("ФрагментЗапускПриложенияЗавершение", ЭтаФорма);
		//
		//Попытка
		//	НачатьЗапускПриложения(ОписаниеОповещения, "cmd /c <nul set /p strTemp=ОТР_КонсольЗапросов.ЗапросВКонсоль(Запрос)|clip");
		//Исключение
...
&НаКлиенте
// Процедура - Фрагмент запуск приложения завершение
//
// Параметры:
//  КодВозврата				 - Число	 - код возврата.
//  ДополнительныеПараметры	 - Произвольный	 - значение, которое было указано при создании объекта ОписаниеОповещения.
//
Процедура ФрагментЗапускПриложенияЗавершение(КодВозврата, ДополнительныеПараметры) Экспорт
