...
Перем МенеджерыДляПлановОбмена Экспорт; // Соответствие, содержащее имена объектов конфигурации для обращения к методам
...
Перем ФлагКомментироватьОбработкуОбъектов Экспорт; // Булево, опрделяет выводить в протокол информацию по обработке
												   // данных или нет.
...
Перем одТипУзлаXML_КонецЭлемента  Экспорт; // Тип("ТипУзлаXML"), соответствует концу элемента XML
Перем одТипУзлаXML_НачалоЭлемента Экспорт; // Тип("ТипУзлаXML"), соответствует началу элемента XML
Перем одТипУзлаXML_Текст          Экспорт; // Тип("ТипУзлаXML"), Соответствует тексту элемента XML.

Перем ЗначениеПустаяДата Экспорт; // Тип("Дата"), хранит значение пустой даты.

Перем одСообщения;             // Соответствие. Ключ - код ошибки,  Значение - описание ошибки.

Перем мСписокМакетовПравилОбмена Экспорт; // Тип("СписокЗначений"), хранит макеты текущей обработки.
...
Перем ЗагруженныеГлобальныеОбъекты Экспорт; // Соответствие. Хранит объекты конфигурации.
Перем ЧислоХранимыхЗагруженныхОбъектов Экспорт;  // Число хранимых загруженных объектов, 
                                          ...
                                          // очищается.
Перем ЗапоминатьЗагруженныеОбъекты Экспорт; // Булево. Хранит факт нужно ли хранить загруженные объекты, или нет.
...
Перем мОбработкаДляЗагрузкиДанных Экспорт; // Хранит объект обработки "УниверсальныйОбменДаннымиXML".

Перем мСоответствиеПустыхЗначенийТипов;
Перем мСоответствиеОписаниеТипов;

Перем мБылиПрочитаныПравилаОбменаПриЗагрузке Экспорт; // Булево. Хранит факт удачного прочтения правил обмена при загрузке.
...
Перем СобытияПослеЗагрузкиПараметров Экспорт; // Структура. Хранит алгоритмы действий после загрузки параметров.
...
Функция ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(КодСообщения, СтрокаОшибки, ИмяПравилаОчисткиДанных, Объект = "", ИмяОбработчика = "")Экспорт
	...
	Если Не ФлагРежимОтладки Тогда
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю(СтрокаСообщенияОбОшибке);
	КонецЕсли;	
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	мМакетОбщиеПроцедурыФункции = Неопределено;
...
Процедура ДополнитьПравилаИнтерфейсамиОбработчиков(СтруктураКонвертация, ТаблицаПКО, ТаблицаПВД, ТаблицаПОД) Экспорт
	...
	мМакетПараметровОбработчиков = Неопределено;
...
// 
Процедура УдалениеОбъектаВыборки(Объект, Правило, Отказ, Свойства=Неопределено, ВходящиеДанные=Неопределено) Экспорт
...
// 
Процедура ПроизвестиЧтениеДанных(ЕстьОшибки, РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
...
//  Нет.
//
Процедура ВыполнитьЗагрузку(ЕстьОшибки) Экспорт
	
	// Загрузку выполняем в транзакции.
	...
	ПроизвестиЧтениеДанных(ЕстьОшибки);
...
//  Нет.
//
Процедура ВыполнитьВыгрузку(ЗавершеноУспешно = Ложь) Экспорт
...
// 
Процедура УдалитьОбъектыТипа(ИмяТипаДляУдаления, Отказ) Экспорт
	...
	Если ИмяТипа = "РегистрСведений" Тогда
		...
		НаборЗаписей = Свойства.Менеджер.СоздатьНаборЗаписей();
		НаборЗаписей.ОбменДанными.Загрузка = Истина;
		
		Попытка
			НаборЗаписей.Записать();
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(29, ОписаниеОшибки(), "УдалениеОбъекта");
			Отказ = Истина;
		КонецПопытки;
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не ОбщегоНазначения.СсылкаСуществует(Выборка.Ссылка) Тогда
				// Битая ссылка. Объекта в базе нет, можно не удалять, продолжаем.
				Продолжить;
			КонецЕсли;
			...
			УдалениеОбъектаВыборки(Объект, Правило, Отказ, Свойства, Неопределено);
