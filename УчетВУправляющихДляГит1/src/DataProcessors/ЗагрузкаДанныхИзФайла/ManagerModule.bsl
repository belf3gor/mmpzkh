...
Процедура СоздатьСписокСправочниковДляЗагрузки(СписокСправочниковДляЗагрузки) Экспорт
	...
	// Заполняем варианты пакетной загрузки по данным АПИ.
	// Удаляем варианты стандартных загрузок, которые присутствуют в пакетной загрузке.
	ВариантыПакетныхЗагрузок = УПЖКХ_ИнтерфейсЗагрузкиДанных.ПолучитьВариантыЗагрузкиДанных();
	
	Для Каждого ТекВариант Из ВариантыПакетныхЗагрузок Цикл
		
		// Заполнение вариантов пакетной загрузки.
		ИнформацияОТипеЗагрузки = Новый Структура;
		ИнформацияОТипеЗагрузки.Вставить("Тип",                        "ПакетнаяЗагрузка");
		ИнформацияОТипеЗагрузки.Вставить("ПолноеИмяОбъектаМетаданных", ТекВариант.ИмяОбъектаДляСопоставления);
		ИнформацияОТипеЗагрузки.Вставить("ВариантЗагрузки" ,           ТекВариант.ВариантЗагрузки);
		
		СписокСправочниковДляЗагрузки.Добавить(ИнформацияОТипеЗагрузки, ТекВариант.Представление);
		
		// Удаляем варианты стандартных загрузок, которые присутствуют в пакетной загрузке.
		СтрокаНаУдаление = ИнформацияОСправочниках.Найти(ТекВариант.ИмяОбъектаДляСопоставления, "ПолноеИмя");
		
		Если СтрокаНаУдаление <> Неопределено Тогда
			ИнформацияОСправочниках.Удалить(СтрокаНаУдаление);
		КонецЕсли;
		
	КонецЦикла;
	
	// Сортируем стандартные загрузки по алфавиту.
	ИнформацияОСправочниках.Сортировать("Представление");
	
	// Квартплата -
	
	// Вносим в список загрузок варианты стандартных загрузок.
...
//
// Квартплата +
Процедура ТабличныйДокументВТаблицуЗначений(ШаблонСДанными, ИнформацияПоКолонкам, АдресЗагруженныхДанных, ПриводитьЗначенияКТипамКолонок = Истина) Экспорт
...
// Квартплата -
	...
	//КонецЦикла;
	Если Не ПриводитьЗначенияКТипамКолонок Тогда
	...
	Для НомерСтроки = ВысотаШапки + 1 По ШаблонСДанными.ВысотаТаблицы Цикл
		...
		Для НомерКолонки = 1 По ШаблонСДанными.ШиринаТаблицы Цикл
			...
			Если НайденнаяКолонка <> Неопределено Тогда
				...
				Если ПриводитьЗначенияКТипамКолонок Тогда
					НоваяСтрока[ИмяКолонки] = ПриведениеЗначениеКТипу(Ячейка.Текст, НайденнаяКолонка.ТипКолонки);
				Иначе
...
Процедура ЗагрузитьФайлВТаблицу(ПараметрыВызоваСервера, АдресХранилища) Экспорт
	...
	// При варианте загрузки "Помещения" не нужно использовать типовую функцию "ПриведениеЗначениеКТипу()".
	ПриводитьЗначенияКТипамКолонок = ПараметрыВызоваСервера.ПриводитьЗначенияКТипамКолонок;
	// Квартплата -
	...
	Иначе
		...
		ТабличныйДокументВТаблицуЗначений(ЗагруженныйШаблонСДанными, ИнформацияПоКолонкам, Адрес, ПриводитьЗначенияКТипамКолонок);
		// Квартплата -
				...
				Если Колонка.Видимость Тогда
					...
					Иначе
						...
						Если Колонка.ИмяКолонки = "ИдентификаторСправочника" Тогда
							ЭлементСправочника["Идентификатор"] = СтрокаТаблицы[Колонка.ИмяКолонки];
						Иначе
							ЭлементСправочника[Колонка.ИмяКолонки] = СтрокаТаблицы[Колонка.ИмяКолонки];
						КонецЕсли;
						// Квартплата -
