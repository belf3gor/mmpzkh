...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Установим условное оформление пакетных вариантов загрузки в списке вариантов.
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("ВидЗагрузкиДанныхЗначение");
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокВариантовЗагрузки.Представление");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
	
	СписокВариантовПакетнойЗагрузки = Новый СписокЗначений;
	Для Каждого ТекСтр Из УПЖКХ_ИнтерфейсЗагрузкиДанных.ПолучитьВариантыЗагрузкиДанных() Цикл
		СписокВариантовПакетнойЗагрузки.Добавить(ТекСтр.Представление);
	КонецЦикла;
	
	ОтборЭлемента.ПравоеЗначение = СписокВариантовПакетнойЗагрузки;
	
	// Цвет выделения - светло-зеленый "Роса".
...
Процедура СписокКолонокСопоставленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// Квартплата +
	СопоставитьПоКолонке.Очистить();
	// Квартплата -
	
	// Квартплата +
	Если ТипЗагрузки = "ПакетнаяЗагрузка" Тогда
		СопоставитьПоКолонке = УПЖКХ_ПолучитьСписокКолонокДляСопоставления(ПараметрыЗагрузки);
	ИначеЕсли СопоставитьПоКолонке.Количество() = 0 Тогда
...
Процедура ПослеВыбораКолонокДляСопоставления(Результат, Параметр) Экспорт
	...
	Если ТипЗагрузки = "ПакетнаяЗагрузка" Тогда
		...
		УПЖКХ_ВыполнитьСопоставлениеОбъектовВАПИ(КоличествоСопоставленныхПоКолонкам);
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	// Квартплата +
	Если ТипЗагрузки = "ПакетнаяЗагрузка" Тогда
		УПЖКХ_ВариантПакетнойЗагрузки = ОписаниеСтрокиВыбора.ВариантЗагрузки;
	Иначе
		УПЖКХ_ВариантПакетнойЗагрузки = "";
	КонецЕсли;
	// Квартплата -
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	// Квартплата +
	Если ПараметрыЗагрузки.ТипЗагрузки = "ПакетнаяЗагрузка" Тогда
		ПараметрыЗагрузки.Вставить("ВариантПакетнойЗагрузки", УПЖКХ_ВариантПакетнойЗагрузки);
		ИнформацияПоКолонкамТаблица = УПЖКХ_ИнтерфейсЗагрузкиДанных.ПолучитьТаблицуПолейЗагружаемыхДанныхПоВариантуЗагрузки(ПараметрыЗагрузки.ВариантПакетнойЗагрузки, ИнформацияПоКолонкам.Выгрузить());
	Иначе
		...
		СтрокаИдектификатора = ИнформацияПоКолонкамТаблица.Найти("Идентификатор", "ИмяКолонки");
		Если СтрокаИдектификатора <> Неопределено Тогда
			СтрокаИдектификатора.ИмяКолонки = "ИдентификаторСправочника";
		КонецЕсли;
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеПроверки()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" ИЛИ ТипЗагрузки = "ТабличнаяЧасть" Тогда
		...
		КонецЕсли;
	// Квартплата +
	ИначеЕсли ТипЗагрузки = "ПакетнаяЗагрузка" Тогда
		УПЖКХ_НайтиИСоздатьСвойстваСсылочногоТипаВТаблицеДанныхНаСервере();
		ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент();
	// Квартплата -
	Иначе
...
Процедура ЗаписатьЗагружаемыеДанныеОтчет()
	...
	ИначеЕсли ТипЗагрузки = "ПакетнаяЗагрузка" Тогда
		...
		ОбновлятьСуществующиеБулево       = ?(ОбновлятьСуществующие = 1, Истина, Ложь);
		СоздаватьЕслиНеСопоставленоБулево = ?(СоздаватьЕслиНеСопоставлено = 1, Истина, Ложь);
		
		УПЖКХ_ИнтерфейсЗагрузкиДанных.ВыполнитьЗагрузкуТаблицыДанных(ПараметрыЗагрузки.ВариантПакетнойЗагрузки,
																СопоставленныеДанные,
																Ложь, 
																ОбновлятьСуществующиеБулево,
																СоздаватьЕслиНеСопоставленоБулево,
																Ложь,
																Истина);
		ЗначениеВРеквизитФормы(СопоставленныеДанные, "ТаблицаСопоставленияДанных");
	// Квартплата -
	...
	Для каждого Элемент Из СопоставитьПоКолонке Цикл
		Если Элемент.Пометка Тогда
			...
			Если Колонка <> Неопределено Тогда
				...
				// Квартплата +
				Если Колонка.ИмяКолонки = "ИдентификаторСправочника" Тогда
					СтрокаУсловие = СтрокаУсловие + РазделительИ + "СправочникСопоставления.Ссылка.Идентификатор" + " =  ТаблицаСопоставления." + Колонка.ИмяКолонки;
					Продолжить;
				КонецЕсли;
				// Квартплата -
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	// Квартплата +
	// При считывании данных из файла, нужно учитывать какой вариант загрузки используется.
	// Параметр необходим для игнорирования использования типовой функции "ПриведениеЗначениеКТипу()" Модуля менеджера.
	ПараметрыВызоваСервера.Вставить("ПриводитьЗначенияКТипамКолонок", Ложь);
	// Квартплата -
...
&НаСервереБезКонтекста
//Возвращает список колонок для сопоставления при пакетной загрузке.
//
Функция УПЖКХ_ПолучитьСписокКолонокДляСопоставления(ПараметрыЗагрузки)
	
	Возврат УПЖКХ_ИнтерфейсЗагрузкиДанных.ПолучитьСписокНаименованийКолонокДляСопоставления(ПараметрыЗагрузки.ВариантПакетнойЗагрузки);
	
КонецФункции

&НаСервере
// Процедура выполняет сопоставление объектов методами АПИ загрузки данных.
//
Процедура УПЖКХ_ВыполнитьСопоставлениеОбъектовВАПИ(КоличествоСопоставленных)
	
	СписокКолонокДляСопоставления = СопоставитьПоКолонке.Скопировать();
	
	Для Каждого ТекЭлемент Из СписокКолонокДляСопоставления Цикл
		Если Не ТекЭлемент.Пометка Тогда
			СписокКолонокДляСопоставления.Удалить(ТекЭлемент);
		КонецЕсли;
	КонецЦикла;
	
	ДанныеДляСопоставления = РеквизитФормыВЗначение("ТаблицаСопоставленияДанных");
	
	УПЖКХ_ИнтерфейсЗагрузкиДанных.ВыполнитьСопоставление(ПараметрыЗагрузки.ВариантПакетнойЗагрузки, ДанныеДляСопоставления, 
												СопоставитьПоКолонке, КоличествоСопоставленных);
	
	ЗначениеВРеквизитФормы(ДанныеДляСопоставления, "ТаблицаСопоставленияДанных");
	
КонецПроцедуры

// Процедура получает данные для актуализации и для полей с примитивными типами данных
// ищет соответствующие объекты в метаданных,
// если поле является объектного типа.
// Возвращает таблицу с данными с найдеными объектами.
//
&НаСервере
Процедура УПЖКХ_НайтиИСоздатьСвойстваСсылочногоТипаВТаблицеДанныхНаСервере()
	
	ТаблицаСопоставления = УПЖКХ_ИнтерфейсЗагрузкиДанных.НайтиИСоздатьСвойстваСсылочногоТипаВТаблицеДанных(ПараметрыЗагрузки.ВариантПакетнойЗагрузки, ШаблонСДанными, ТаблицаСопоставления);
	
	// Добавление стандартных колонок в таблицу сопоставления.
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ИмяОбъектаСопоставления);
	ОписаниеТипаОбъектаСопоставления = ОписаниеТипаПоМетаданным(ИмяОбъектаСопоставления);
	
	Синоним = ОбъектМетаданных.Синоним;
	СтруктураОбъектаСопоставления = Новый Структура("ОписаниеТипаОбъектаСопоставления, Синоним", ОписаниеТипаОбъектаСопоставления, Синоним);
	ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ТаблицаСопоставления, СтруктураОбъектаСопоставления, Ложь, Истина, Ложь, Истина);
...
КонецПроцедуры // УПЖКХ_НайтиИСоздатьСвойстваСсылочногоТипаВТаблицеДанныхНаСервере()
