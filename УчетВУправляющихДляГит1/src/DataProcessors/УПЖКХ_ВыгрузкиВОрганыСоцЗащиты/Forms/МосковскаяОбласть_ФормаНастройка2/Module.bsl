...
Процедура КомандаТаблицаБлагоустройствЗаполнитьПоУмолчанию(Команда)
	...
	Объект.МосковскаяОбласть2_СоответствиеВидовБлагоустройств.Очистить();
	ТаблицаБлагоустройствЗаполнитьПоУмолчаниюНаСервере();
...
Процедура КомандаТаблицаНастроекЗаполнитьПоУмолчанию(Команда)
	...
	Если Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.Количество() > 0 Тогда
...
Процедура КомандаТаблицаНастроекЗаполнитьПоУмолчаниюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.Очистить();
...
Процедура ТаблицаНастроекЗаполнитьПоУмолчаниюНаСервере()
	...
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ЗаполнитьТаблицуВидовУслуг();
	ЗначениеВРеквизитФормы(ОбработкаОбъект, "Объект");
...
Процедура ТаблицаНастроекПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		НайденныеСтроки = Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.НайтиСтроки(Отбор);
...
Процедура ТП_ЖКУ_ВБазеПередУдалением(Элемент, Отказ)
	...
	Иначе
		...
		СтрокиУдаления = Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.НайтиСтроки(Новый Структура("УслугаВБазе", Элемент.ТекущиеДанные.УслугаВБазе));
		Для Каждого СтрокаУдаления Из СтрокиУдаления Цикл
			Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.Удалить(СтрокаУдаления);
		КонецЦикла;
...
Процедура ТП_ЖКУ_ВБазеПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Элемент.ТекущиеДанные.УслугаВБазе.Пустая() Тогда
		...
		Если Не НоваяСтрока Тогда
			...
			СтрокиУдаления = Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.НайтиСтроки(Новый Структура("УслугаВБазе", ПредыдущаяУслуга));
			Для Каждого СтрокаУдаления Из СтрокиУдаления Цикл
				Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.Удалить(СтрокаУдаления);
			КонецЦикла;
...
Процедура ТП_ЖКУ_ВБазеПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		Если НоваяСтрока Тогда
			...
			СтрокаСоответствия = Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.Добавить();
		...
		Иначе
			...
			СтрокиСоответствия = Объект.МосковскаяОбласть2_СоответствиеВидовУслуг.НайтиСтроки(Новый Структура("ВидУслугиВОСЗН,УслугаВБазе", Элементы.ТаблицаНастроек.ТекущиеДанные.ВидУслугиВОСЗН, ПредыдущаяУслуга));
...
Процедура ТП_ЖКУ_ВБазеУслугаВБазеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Для Каждого Строка Из Объект.МосковскаяОбласть2_СоответствиеВидовУслуг Цикл
