...
&НаКлиенте
// Процедура - обработчик команды "УПЖКХ_Заполнить".
//
Процедура УПЖКХ_Заполнить(Команда)
	УПЖКХ_ЗаполнитьНаСервере();
КонецПроцедуры
...
&НаСервере
// Выполняет заполнение списка ключевых операций значениями операций ЖКХ.
//
Процедура УПЖКХ_ЗаполнитьНаСервере()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	НЕ КлючевыеОперации.ПометкаУдаления
	                      |	И КлючевыеОперации.Имя В(&МассивИменКлючевыхОпераций)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	КлючевыеОперации.Наименование");
	
	ТаблицаКлючевыхОпераций    = УПЖКХ_ОбщегоНазначенияСервер.ПолучитьТаблицуКлючевыхОперацийЖКХ();
	МассивИменКлючевыхОпераций = ТаблицаКлючевыхОпераций.ВыгрузитьКолонку("ИмяКлючевойОперации");
	
	Запрос.УстановитьПараметр("МассивИменКлючевыхОпераций", МассивИменКлючевыхОпераций);
