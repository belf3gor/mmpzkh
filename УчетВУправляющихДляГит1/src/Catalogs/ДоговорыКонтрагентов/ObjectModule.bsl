	...
	Иначе
		...
		Если НеобходимаПроверка Тогда
			...
			Если ЕстьПроведенныеДокументыПоДоговору Тогда
				...
				СписокРеквизитов = Новый Структура("ВалютаВзаиморасчетов, ВидДоговора, Организация, УчетАгентскогоНДС, ЭлектронныеУслуги, УчетАгентскогоНДСПокупателем, СчетаФактурыОтИмениОрганизации, УПЖКХ_ВариантПоставкиУслуг, УПЖКХ_ОплатаЧерезУК",
							...
							// Квартплата +
							НСтр("ru = 'Вариант поставки услуг'"),
							НСтр("ru = 'Оплата через управляющую компанию'"));
							// Квартплата -
		...
		// Квартплата +
		Если ДоговорИспользованВУправленииДоговорамиПоставщиков() Тогда
		
			ТекстСообщения = НСтр("ru = 'Существуют документы, проведенные по договору %1.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Наименование);
			ТекстСообщения = ТекстСообщения + НСтр("ru = '
				|Реквизит %2 не может быть изменен.'");
			СообщитьОНекорректномРеквизите("УПЖКХ_ВариантПоставкиУслуг", НСтр("ru = 'Вариант поставки услуг'"),ТекстСообщения, Отказ);
			СообщитьОНекорректномРеквизите("УПЖКХ_ОплатаЧерезУК", НСтр("ru = 'Оплата через управляющую компанию'"),ТекстСообщения, Отказ);
		КонецЕсли;
		// Квартплата -
...
// Квартплата +
// Получает результат проверки использования договора в ранее проведенных документах.
Функция ДоговорИспользованВУправленииДоговорамиПоставщиков()
	
	РеквизитыДоговораИзИБ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, 
		"УПЖКХ_ВариантПоставкиУслуг, УПЖКХ_ОплатаЧерезУК");
	Результат = Ложь;
	
	Если УПЖКХ_ВариантПоставкиУслуг <> РеквизитыДоговораИзИБ.УПЖКХ_ВариантПоставкиУслуг
	 ИЛИ УПЖКХ_ОплатаЧерезУК <> РеквизитыДоговораИзИБ.УПЖКХ_ВариантПоставкиУслуг Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	УПЖКХ_ПоставщикиУслугСрезПоследних.Активность КАК Активность
			|ИЗ
			|	РегистрСведений.УПЖКХ_ПоставщикиУслуг.СрезПоследних(, ДоговорПоставщикаУслуг = &Договор) КАК УПЖКХ_ПоставщикиУслугСрезПоследних";
