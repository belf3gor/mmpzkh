...
// Процедура заполняет параметры периода в зависимости от выбранного варианта отчета.
//
Процедура УстановитьПараметрыПериодаПоВариантуОтчета(Форма)
	
	Элементы = Форма.Элементы;
	Отчет    = Форма.Отчет;
	
	// Период отчета не сохраняем, а всегда определяем его заново.
	ТекущаяДата = УПЖКХ_ТиповыеМетодыКлиентСервер.ПолучитьРабочуюДату();
	
	Если Форма.КлючТекущегоВарианта = "ЗарегистрированныеПрибышиеЗаПериод" Тогда
		
		// Для данного варианта отчета можно указать произвольный период отчета.
		...
		УПЖКХ_ТиповыеМетодыКлиентСервер.ЗаполнитьСписокВыбораВидПериода(
			...
			Элементы.ВидПериода.СписокВыбора, Форма.ВидПериода);
		
	Иначе
		
		// Для остальных вариантов отчета можно указать только конкретный день.
		Элементы.ВидПериода.СписокВыбора.Очистить();
		Элементы.ВидПериода.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.День"));
		
		Отчет.НачалоПериода = ТекущаяДата;
		Отчет.КонецПериода  = КонецДня(Отчет.НачалоПериода);
		
		// Установим вид периода "День".
		Форма.ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.День");
...
Процедура ДобавитьПоляВНаборКомпоновкиНаСервере()
	...
	Отчеты.КВП_СписокЗарегистрированныхИПроживающих.ДобавитьПоляВНаборКомпоновки(ПараметрыОтчета,
...
Процедура ПриОткрытии(Отказ)
	...
	// Установка параметров периода отчета.
	УстановитьПараметрыПериодаПоВариантуОтчета(ЭтаФорма);
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	// Изменение параметров отбора производится для предопределенных вариантов отчета.
	Если КлючТекущегоВарианта = "ДанныеОРегистрацииПроживании"
	 ИЛИ КлючТекущегоВарианта = "ЗарегистрированныеПрибышиеЗаПериод"
	 ИЛИ КлючТекущегоВарианта = "ПаспортныеДанные"
	 ИЛИ КлючТекущегоВарианта = "СостояниеЖильцов" Тогда
		
		// Очистка ранее установленных отборов.
		Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Очистить();
		
		// Добавление отбора по организации.
...
Процедура ПриЗагрузкеВариантаНаСервере(Настройки)
	...
	Отчеты.КВП_СписокЗарегистрированныхИПроживающих.ПередКомпоновкойМакета(ПодготовитьПараметрыОтчетаНаСервере(),
																			ПолучитьИзВременногоХранилища(СхемаКомпоновкиДанных),
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("ИмяВариантаОтчета"                , КлючТекущегоВарианта);
	ПараметрыОтчета.Вставить("ПредставлениеВариантаОтчета"      , ПредставлениеТекущегоВарианта);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Список зарегистрированных и проживающих: " + Форма.ПредставлениеТекущегоВарианта;
