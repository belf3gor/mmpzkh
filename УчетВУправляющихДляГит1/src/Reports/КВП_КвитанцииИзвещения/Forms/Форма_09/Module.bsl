...
Процедура СохранениеРеквизитов()
	...
	СтруктураПараметров.Вставить("БанковскийСчет",                    Отчет.БанковскийСчет);
...
Процедура ЗаполнитьТаблицуОбщийСписокУслуг()
	...
	ВсеУслуги = Отчеты.КВП_КвитанцииИзвещения.ПолучитьВсеУслуги();
	...
	Если Отчет.ОбщийСписокУслуг.Количество() = 0 
		...
		Отчет.ОбщийСписокУслуг.Загрузить(ВсеУслуги);
	ИначеЕсли ВсеУслуги.Количество() <> (Отчет.ОбщийСписокУслуг.Количество() 
		...
		Для Сч = 1 По КоличествоСтр Цикл
			...
			Если Отчет.ОбщийСписокУслуг.НайтиСтроки(Новый Структура("Услуга", СтрВсехУслуг.Услуга)).Количество() > 0
			 Или Отчет.УслугиКоммунальные.НайтиСтроки(Новый Структура("Услуга", СтрВсехУслуг.Услуга)).Количество() > 0
			 Или Отчет.УслугиСодержанияРемонта.НайтиСтроки(Новый Структура("Услуга", СтрВсехУслуг.Услуга)).Количество() > 0 Тогда
		...
		Если ЕстьИзменения Тогда
			УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке(
				"Изменен список услуг, необходимо проверить разделение услуг по видам на закладке ""Настройка"".");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВСодержание(Команда)
	...
	Попытка
		...
		Если (Не ТекСтрокаУслуг = Неопределено) И (ЗначениеЗаполнено(ЗначениеЗаполнено(ТекСтрокаУслуг.Услуга))) Тогда
			НовСтр = Отчет.УслугиСодержанияРемонта.Добавить();
			НовСтр.Услуга = ТекСтрокаУслуг.Услуга;
