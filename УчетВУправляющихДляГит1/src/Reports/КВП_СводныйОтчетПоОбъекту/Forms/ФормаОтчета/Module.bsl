...
Процедура УстановитьВидимость()
	...
	Элементы.ТипОбъектов.Видимость = Отчет.ПечататьРаздельноПоКаждомуОбъекту;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Если настройки были изменены, то они будут загружены при формировании отчета.
...
&НаКлиенте
// Обработчик события "ПриИзменении" поля "ПечататьРаздельноПоКаждомуОбъекту".
Процедура ПечататьРаздельноПоКаждомуОбъектуПриИзменении(Элемент)
...
Процедура СформироватьОтчет(Команда)
	...
	Если Отчет.ПечататьРаздельноПоКаждомуОбъекту
	   И Не ЗначениеЗаполнено(Отчет.Объект) Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьРезультатВопросаПродолжитьВыполнение", ЭтаФорма);
		ТекстВопроса = "Формирование отчета по всем объектам может занять продолжительное время!" + Символы.ПС + "Продолжить?";
...
&НаКлиенте
// Обработчик результата вопроса, вызванного в процедуре "СформироватьОтчет()".
Процедура ОбработатьРезультатВопросаПродолжитьВыполнение(Ответ, ДопПараметры) Экспорт 
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		СформироватьОтчетПродолжение();
	КонецЕсли;
	
КонецПроцедуры // ОбработатьРезультатВопросаПродолжитьВыполнение()

&НаКлиенте
// Процедура продолжение процедуры "СформироватьОтчет()".
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("ПечататьРаздельноПоКаждомуОбъекту", Отчет.ПечататьРаздельноПоКаждомуОбъекту);
	ПараметрыОтчета.Вставить("ТипОбъектов"						, Отчет.ТипОбъектов);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Сводный отчет по объекту"
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	Если ИБФайловая Тогда
		...
		Отчеты.КВП_СводныйОтчетПоОбъекту.СформироватьОтчет(ПараметрыОтчета, АдресХранилища);
		РезультатВыполнения = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		РезультатВыполнения = УПЖКХ_ТиповыеМетодыСервер.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор,
			"Отчеты.КВП_СводныйОтчетПоОбъекту.СформироватьОтчет",
			ПараметрыОтчета,
