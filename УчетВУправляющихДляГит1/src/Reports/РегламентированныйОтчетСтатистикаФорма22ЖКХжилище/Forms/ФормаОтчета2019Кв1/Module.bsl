...
Процедура ЗаполнитьАвто(Отказ)
	...
	СтруктураПараметров = 
		УПЖКХ_ТиповыеМетодыВызовСервера.ХранилищеОбщихНастроекЗагрузить("СтатистикаФорма22Жилище_НастройкиОтчета");
	
	Если Не ТипЗнч(СтруктураПараметров) = Тип("Структура") Или Не СтруктураПараметров.Свойство("ТаблицаСоответствияУслуг")
	 Или Не СтруктураПараметров.Свойство("ТаблицаСоответствияУслуг") Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Для Каждого Услуга Из СтруктураПараметров.ТаблицаСоответствияУслуг Цикл
		Если Услуга.ВидУслуги = "ЗахоронениеТБО" Тогда
			ЗахоронениеТБО = Услуга;
		ИначеЕсли Услуга.ВидУслуги = "ПрочиеУслуги" Тогда
			ПрочиеУслуги = Услуга;
		КонецЕсли;
	КонецЦикла;
	
	Если ЗахоронениеТБО <> Неопределено Тогда
		СтруктураПараметров.ТаблицаСоответствияУслуг.Удалить(ЗахоронениеТБО);
	КонецЕсли;
	Если ПрочиеУслуги <> Неопределено Тогда
		СтруктураПараметров.ТаблицаСоответствияУслуг.Удалить(ПрочиеУслуги);
	КонецЕсли;
	...
	ЗапросСредстваНаКапРемонт = Новый Запрос;
	...
	Для Каждого СтрокаВидаУслуги ИЗ СтруктураРеквизитовФормы.мТаблицаСоответствияУслугЖилищные Цикл
		...
		Для Каждого СтрокаУслуги ИЗ СтрокаВидаУслуги.СписокУслуг Цикл
			...
			Если СтрокаВидаУслуги.ВидУслуги = "КапитальныйРемонт" Тогда
				СписокУслугКапРемонта.Добавить(СтрокаУслуги.Значение);
			КонецЕсли;
	...
	Если УслугиНастройкиУказаны Тогда
		...
		Для Каждого СтрокаОбъема ИЗ ТаблицаОбъемов Цикл
			...
			Если СтрокаОбъема.ВидУслуги = "ХолоднаяВода" Тогда
				...
				// раздел 1, строкаи 01-02, графа 03
				ТабДокумент.Области["П010103"].Значение = ТабДокумент.Области["П010103"].Значение + СтрокаОбъема.Количество;
				
				Если СтрокаОбъема.Многоквартирное Тогда
					ТабДокумент.Области["П010203"].Значение = ТабДокумент.Области["П010203"].Значение + СтрокаОбъема.КоличествоОПУ;
				КонецЕсли;
			...
			ИначеЕсли СтрокаОбъема.ВидУслуги = "ГорячаяВода" Тогда
				...
				Если СтрокаОбъема.Многоквартирное Тогда
					ТабДокумент.Области["П010204"].Значение = ТабДокумент.Области["П010204"].Значение + СтрокаОбъема.КоличествоОПУ;
				КонецЕсли;
			...
			ИначеЕсли СтрокаОбъема.ВидУслуги = "Водоотведение" Тогда
				...
				Если СтрокаОбъема.Многоквартирное Тогда
					ТабДокумент.Области["П010205"].Значение = ТабДокумент.Области["П010205"].Значение + СтрокаОбъема.КоличествоОПУ;
				КонецЕсли;
			
			ИначеЕсли СтрокаОбъема.ВидУслуги = "Отопление" Тогда
				
				// раздел 1, строка 01, графа 06
			...
			ИначеЕсли СтрокаОбъема.ВидУслуги = "Электроэнергия" Тогда
				...
				Если СтрокаОбъема.Многоквартирное Тогда
					ТабДокумент.Области["П010208"].Значение = ТабДокумент.Области["П010208"].Значение + СтрокаОбъема.КоличествоОПУ;
				КонецЕсли;
				
			ИначеЕсли СтрокаОбъема.ВидУслуги = "СетевойГаз" Тогда
				
				// раздел 1, строка 01, графа 09
	...
	Если УслугиНастройкиУказаны ИЛИ УслугиЖилищныеНастройкиУказаны Тогда
		...
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			...
			ТабДокумент.Области["П023504"].Значение = СуммаДолга;
		КонецЕсли;
		
		// раздел 2, строка 36, графа 03
		НайденнаяСтрока = ТаблицаОстатков.Найти("ГорячаяВода", "ВидУслуги");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаДолга = ПеревестиВТысячи(НайденнаяСтрока.СуммаДолга);
			
			ТабДокумент.Области["П023604"].Значение = СуммаДолга;
		КонецЕсли;
		
		// раздел 2, строка 37, графа 03
		НайденнаяСтрока = ТаблицаОстатков.Найти("Водоотведение", "ВидУслуги");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаДолга = ПеревестиВТысячи(НайденнаяСтрока.СуммаДолга);
			
			ТабДокумент.Области["П023704"].Значение = СуммаДолга;
		КонецЕсли;
		
		// раздел 2, строка 38, графа 03
		НайденнаяСтрока = ТаблицаОстатков.Найти("Отопление", "ВидУслуги");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаДолга = ПеревестиВТысячи(НайденнаяСтрока.СуммаДолга);
			
			ТабДокумент.Области["П023804"].Значение = СуммаДолга;
		КонецЕсли;
		
		// раздел 2, строка 39, графа 03
		НайденнаяСтрока = ТаблицаОстатков.Найти("Электроэнергия", "ВидУслуги");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаДолга = ПеревестиВТысячи(НайденнаяСтрока.СуммаДолга);
			
			ТабДокумент.Области["П023904"].Значение = СуммаДолга;
		КонецЕсли;
		
		// раздел 2, строка 40, графа 03
		СуммаДолгаГазоснобжение = 0;
		НайденнаяСтрока = ТаблицаОстатков.Найти("СетевойГаз", "ВидУслуги");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаДолгаГазоснобжение = СуммаДолгаГазоснобжение + НайденнаяСтрока.СуммаДолга;
		КонецЕсли;
		...
		// Округление
		СуммаДолгаГазоснобжение = ПеревестиВТысячи(СуммаДолгаГазоснобжение);
		
		ТабДокумент.Области["П024004"].Значение = СуммаДолгаГазоснобжение;
		
		НайденнаяСтрока = ТаблицаОстатков.Найти("СодержаниеИРемонт", "ВидУслуги");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаДолга = ПеревестиВТысячи(НайденнаяСтрока.СуммаДолга);
			
			ТабДокумент.Области["П024304"].Значение = СуммаДолга;
		КонецЕсли;
	...
	Если УслугиЖилищныеНастройкиУказаны Тогда
		...
		|			Запись,
		|			Организация = &Организация
		|				И Услуга В (&СписокУслугКапРемонта)
		...
		|				И РазделУчета В (&РазделыУчета)
		|				И ЛицевойСчет ССЫЛКА Справочник.КВП_ЛицевыеСчета) КАК УПЖКХ_НачисленияОбороты
		|
		|СГРУППИРОВАТЬ ПО
		...
		|		ПО втНачисления.Здание = втОплаты.Здание
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	УПЖКХ_СведенияОбОбслуживающихЗданияОрганизацияхСрезПоследних.СпособФормированияФондаКапитальногоРемонта КАК СпособФормированияФондаКапитальногоРемонта,
		|	СУММА(ЕСТЬNULL(втОплатыНачисления.СуммаНачисления, 0)) КАК СуммаНачисления,
		|	СУММА(ЕСТЬNULL(втОплатыНачисления.СуммаОплаты, 0)) КАК СуммаОплаты
		|ИЗ
		|	РегистрСведений.УПЖКХ_СведенияОбОбслуживающихЗданияОрганизациях.СрезПоследних(
		|			&КонецПериода,
		|			НаходитсяНаОбслуживании
		|				И Организация = &Организация) КАК УПЖКХ_СведенияОбОбслуживающихЗданияОрганизацияхСрезПоследних
		|		ЛЕВОЕ СОЕДИНЕНИЕ втОплатыНачисления КАК втОплатыНачисления
		|		ПО (втОплатыНачисления.Здание = УПЖКХ_СведенияОбОбслуживающихЗданияОрганизацияхСрезПоследних.Здание)
		|
		|СГРУППИРОВАТЬ ПО
		|	УПЖКХ_СведенияОбОбслуживающихЗданияОрганизацияхСрезПоследних.СпособФормированияФондаКапитальногоРемонта";
		
		ЗапросСредстваНаКапРемонт.Текст = ТекстЗапроса;
		ЗапросСредстваНаКапРемонт.УстановитьПараметр("НачалоПериода",         СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		ЗапросСредстваНаКапРемонт.УстановитьПараметр("КонецПериода",          СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ЗапросСредстваНаКапРемонт.УстановитьПараметр("Организация",           СтруктураРеквизитовФормы.Организация);
		ЗапросСредстваНаКапРемонт.УстановитьПараметр("СписокУслугКапРемонта", СписокУслугКапРемонта);
		ЗапросСредстваНаКапРемонт.УстановитьПараметр("РазделыУчета",          РазделыУчетаНачислений);
		ЗапросСредстваНаКапРемонт.УстановитьПараметр("ВидыНачислений",        ВидыНачислений);
		
		ТаблицаНачисленийИОплатПоКапРемонту = ЗапросСредстваНаКапРемонт.Выполнить().Выгрузить();
		
		НайденнаяСтрока = ТаблицаНачисленийИОплатПоКапРемонту.Найти(Перечисления.УПЖКХ_СпособыФормированияФондаКапитальногоРемонта.НаСчетеРегиональногоОператора, "СпособФормированияФондаКапитальногоРемонта");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаНачисления = ПеревестиВТысячи(НайденнаяСтрока.СуммаНачисления);
			СуммаОплаты     = ПеревестиВТысячи(НайденнаяСтрока.СуммаОплаты);
			
			ТабДокумент.Области["П035904"].Значение = СуммаНачисления;
			ТабДокумент.Области["П036004"].Значение = СуммаОплаты;
		КонецЕсли;
		
		НайденнаяСтрока = ТаблицаНачисленийИОплатПоКапРемонту.Найти(Перечисления.УПЖКХ_СпособыФормированияФондаКапитальногоРемонта.НаСпециальномСчете, "СпособФормированияФондаКапитальногоРемонта");
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			СуммаНачисления = ПеревестиВТысячи(НайденнаяСтрока.СуммаНачисления);
			СуммаОплаты     = ПеревестиВТысячи(НайденнаяСтрока.СуммаОплаты);
			
			ТабДокумент.Области["П035905"].Значение = СуммаНачисления;
			ТабДокумент.Области["П036005"].Значение = СуммаОплаты;
		КонецЕсли;
	...
	Если УслугиНастройкиУказаны ИЛИ УслугиЖилищныеНастройкиУказаны Тогда
		...
		// Состояние("Заполнение раздела 4...");
		...
		|				И ВидНачисления = ЗНАЧЕНИЕ(Перечисление.УПЖКХ_ВидыНачислений.НачислениеПоОПУ)
		|				И РазделУчета В (&РазделыУчета)
		|				И Услуга.КоммунальныйРесурс <> ЗНАЧЕНИЕ(Перечисление.УПЖКХ_ВидыРесурсов.ПустаяСсылка)
		|				И ЛицевойСчет.Адрес.Владелец.Многоквартирное) КАК УПЖКХ_НачисленияОбороты
		|
		|СГРУППИРОВАТЬ ПО
		...
		|			ЛицевойСчет В
		|				(ВЫБРАТЬ
		|					втНачисленияОплатыПоВидамУслуг.ЛицевойСчет
		|				ИЗ
		|					втНачисленияОплатыПоВидамУслуг КАК втНачисленияОплатыПоВидамУслуг)) КАК КВП_ЛицевыеСчетаСрезПоследних
		|ГДЕ
		...
		|					втНачисленияОплатыПоВидамУслуг.ЛицевойСчет
		|				ИЗ
		|					втНачисленияОплатыПоВидамУслуг КАК втНачисленияОплатыПоВидамУслуг)) КАК КВП_УчетКоличестваЖильцовПоЛицевымСчетамСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|			&КонецПериода,
		|			ДатаИзменения <= &КонецПериода
		|				И ЛицевойСчет В
		|					(ВЫБРАТЬ
		|						втНачисленияОплатыПоВидамУслуг.ЛицевойСчет
		|					ИЗ
		|						втНачисленияОплатыПоВидамУслуг КАК втНачисленияОплатыПоВидамУслуг)) КАК УПЖКХ_СведенияОПроживающихСрезПоследних
		|ГДЕ
		...
		|
		|ВЫБРАТЬ
		|	втНачисленияОДН.ЛицевойСчет,
		|	втНачисленияОДН.Здание,
		|	втНачисленияОДН.ВидУслуги,
		|	СУММА(ЕСТЬNULL(втНачисленияОДН.СуммаНачисленияОборот, 0)),
		|	0
		|ИЗ
		|	втНачисленияОДН КАК втНачисленияОДН
		|
		|СГРУППИРОВАТЬ ПО
		|	втНачисленияОДН.ЛицевойСчет,
		|	втНачисленияОДН.Здание,
		...
		Для Каждого СтрокаСведений ИЗ ТаблицаСведений Цикл
			...
			Если СтрокаСведений.ВидУслуги = "Наем" Тогда
				...
				// раздел 4, строка 64, графы 3, 4, 8
				ТабДокумент.Области["П046404"].Значение = ТабДокумент.Области["П046404"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П046405"].Значение = ТабДокумент.Области["П046405"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П046409"].Значение = ТабДокумент.Области["П046409"].Значение + СтрокаСведений.Площадь;
			...
			ИначеЕсли СтрокаСведений.ВидУслуги = "СодержаниеИРемонт" Тогда
				...
				Если СтрокаСведений.ЕстьБлагоустройства Тогда
					...
					// раздел 4, строка 66, графы 3, 4, 8
					ТабДокумент.Области["П046604"].Значение = ТабДокумент.Области["П046604"].Значение + СтрокаСведений.СуммаНачисления;
					ТабДокумент.Области["П046605"].Значение = ТабДокумент.Области["П046605"].Значение + СтрокаСведений.СуммаОплаты;
					ТабДокумент.Области["П046609"].Значение = ТабДокумент.Области["П046609"].Значение + СтрокаСведений.Площадь;
					
				Иначе
					
					// раздел 4, строка 67, графы 3, 4, 8
					ТабДокумент.Области["П046704"].Значение = ТабДокумент.Области["П046704"].Значение + СтрокаСведений.СуммаНачисления;
					ТабДокумент.Области["П046705"].Значение = ТабДокумент.Области["П046705"].Значение + СтрокаСведений.СуммаОплаты;
					ТабДокумент.Области["П046709"].Значение = ТабДокумент.Области["П046709"].Значение + СтрокаСведений.Площадь;
					
				КонецЕсли;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "СОИ" Тогда
				
				// раздел 4, строка 68, графы 3, 4, 8
					ТабДокумент.Области["П046804"].Значение = ТабДокумент.Области["П046804"].Значение + СтрокаСведений.СуммаНачисления;
					ТабДокумент.Области["П046805"].Значение = ТабДокумент.Области["П046805"].Значение + СтрокаСведений.СуммаОплаты;
					ТабДокумент.Области["П046809"].Значение = ТабДокумент.Области["П046809"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "ВывозТО" Тогда
				
				// раздел 4, строка 83, графы 3, 4, 8
				ТабДокумент.Области["П048304"].Значение = ТабДокумент.Области["П048304"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П048305"].Значение = ТабДокумент.Области["П048305"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П048309"].Значение = ТабДокумент.Области["П048309"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "КапитальныйРемонт" Тогда
				
				// раздел 4, строка 69, графы 3, 4, 8
				ТабДокумент.Области["П046904"].Значение = ТабДокумент.Области["П046904"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П046905"].Значение = ТабДокумент.Области["П046905"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П046909"].Значение = ТабДокумент.Области["П046909"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "ХолоднаяВода" Тогда
				
				// раздел 4, строка 71, графы 3, 4, 8
				ТабДокумент.Области["П047104"].Значение = ТабДокумент.Области["П047104"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047105"].Значение = ТабДокумент.Области["П047105"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047109"].Значение = ТабДокумент.Области["П047109"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "Водоотведение" Тогда
				
				// раздел 4, строка 72, графы 3, 4, 8
				ТабДокумент.Области["П047204"].Значение = ТабДокумент.Области["П047204"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047205"].Значение = ТабДокумент.Области["П047205"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047209"].Значение = ТабДокумент.Области["П047209"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "ГорячаяВода" Тогда
				
				// раздел 4, строка 73, графы 3, 4, 8
				ТабДокумент.Области["П047304"].Значение = ТабДокумент.Области["П047304"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047305"].Значение = ТабДокумент.Области["П047305"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047309"].Значение = ТабДокумент.Области["П047309"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "Отопление" Тогда
				
				// раздел 4, строка 74, графы 3, 4, 8
				ТабДокумент.Области["П047404"].Значение = ТабДокумент.Области["П047404"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047405"].Значение = ТабДокумент.Области["П047405"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047409"].Значение = ТабДокумент.Области["П047409"].Значение + СтрокаСведений.Площадь;
				
				Если ОтчетностьЗаГод И СтрокаСведений.Многоквартирное Тогда
					
					// Справочная информация, строки 86, графы 3, 5
					ТабДокумент.Области["П048603"].Значение = ТабДокумент.Области["П048603"].Значение + СтрокаСведений.Площадь;
					ТабДокумент.Области["П048605"].Значение = ТабДокумент.Области["П048605"].Значение + СтрокаСведений.ОбщаяПлощадь;
					
				КонецЕсли;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "Электроэнергия" Тогда
				
				// раздел 4, строка 75, графы 3, 4, 8
				ТабДокумент.Области["П047504"].Значение = ТабДокумент.Области["П047504"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047505"].Значение = ТабДокумент.Области["П047505"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047509"].Значение = ТабДокумент.Области["П047509"].Значение + СтрокаСведений.Площадь;
				
				Если ОтчетностьЗаГод И СтрокаСведений.Многоквартирное Тогда
					
					// Справочная информация, строка 85, графы 4, 5
					ТабДокумент.Области["П048504"].Значение = ТабДокумент.Области["П048504"].Значение + СтрокаСведений.ПлощадьМестОбщегоПользования;
					ТабДокумент.Области["П048505"].Значение = ТабДокумент.Области["П048505"].Значение + СтрокаСведений.ОбщаяПлощадь;
					
				КонецЕсли;
				
				Если СтрокаСведений.ТипПлиты = Перечисления.КВП_Плита.Газовая Тогда
					
					// раздел 4, строка 76, графы 3, 4, 8
					ТабДокумент.Области["П047604"].Значение = ТабДокумент.Области["П047604"].Значение + СтрокаСведений.СуммаНачисления;
					ТабДокумент.Области["П047605"].Значение = ТабДокумент.Области["П047605"].Значение + СтрокаСведений.СуммаОплаты;
					ТабДокумент.Области["П047609"].Значение = ТабДокумент.Области["П047609"].Значение + СтрокаСведений.Площадь;
					
				ИначеЕсли СтрокаСведений.ТипПлиты = Перечисления.КВП_Плита.Электрическая Тогда
					
					// Раздел 4, строка 77, графы 3, 4, 8
					ТабДокумент.Области["П047704"].Значение = ТабДокумент.Области["П047704"].Значение + СтрокаСведений.СуммаНачисления;
					ТабДокумент.Области["П047705"].Значение = ТабДокумент.Области["П047705"].Значение + СтрокаСведений.СуммаОплаты;
					ТабДокумент.Области["П047709"].Значение = ТабДокумент.Области["П047709"].Значение + СтрокаСведений.Площадь;
					
				КонецЕсли;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "СетевойГаз" Тогда
				
				// Раздел 4, строка 78, графы 3, 4, 8
				ТабДокумент.Области["П047804"].Значение = ТабДокумент.Области["П047804"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047805"].Значение = ТабДокумент.Области["П047805"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047809"].Значение = ТабДокумент.Области["П047809"].Значение + СтрокаСведений.Площадь;
				
			ИначеЕсли СтрокаСведений.ВидУслуги = "СжиженныйГаз" Тогда
				
				// Раздел 4, строка 79, графы 3, 4, 8
				ТабДокумент.Области["П047904"].Значение = ТабДокумент.Области["П047904"].Значение + СтрокаСведений.СуммаНачисления;
				ТабДокумент.Области["П047905"].Значение = ТабДокумент.Области["П047905"].Значение + СтрокаСведений.СуммаОплаты;
				ТабДокумент.Области["П047909"].Значение = ТабДокумент.Области["П047909"].Значение + СтрокаСведений.Площадь;
		...
		// Округление
		ТабДокумент.Области["П046404"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046404"].Значение);
		ТабДокумент.Области["П046405"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046405"].Значение);
		
		ТабДокумент.Области["П046604"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046604"].Значение);
		ТабДокумент.Области["П046605"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046605"].Значение );
		
		ТабДокумент.Области["П046704"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046704"].Значение);
		ТабДокумент.Области["П046705"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046705"].Значение);
		
		ТабДокумент.Области["П046804"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046804"].Значение);
		ТабДокумент.Области["П046805"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046805"].Значение);
		
		ТабДокумент.Области["П048304"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П048304"].Значение);
		ТабДокумент.Области["П048305"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П048305"].Значение);
		
		ТабДокумент.Области["П046904"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046904"].Значение);
		ТабДокумент.Области["П046905"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П046905"].Значение);
		
		ТабДокумент.Области["П047104"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047104"].Значение);
		ТабДокумент.Области["П047105"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047105"].Значение);
		
		ТабДокумент.Области["П047204"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047204"].Значение);
		ТабДокумент.Области["П047205"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047205"].Значение);
		
		ТабДокумент.Области["П047304"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047304"].Значение);
		ТабДокумент.Области["П047305"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047305"].Значение);
		
		ТабДокумент.Области["П047404"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047404"].Значение);
		ТабДокумент.Области["П047405"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047405"].Значение);
		
		ТабДокумент.Области["П047504"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047504"].Значение);
		ТабДокумент.Области["П047505"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047505"].Значение);
		
		ТабДокумент.Области["П047604"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047604"].Значение);
		ТабДокумент.Области["П047605"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047605"].Значение);
		
		ТабДокумент.Области["П047704"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047704"].Значение);
		ТабДокумент.Области["П047705"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047705"].Значение);
		
		ТабДокумент.Области["П047804"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047804"].Значение);
		ТабДокумент.Области["П047805"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047805"].Значение);
		
		ТабДокумент.Области["П047904"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047904"].Значение);
		ТабДокумент.Области["П047905"].Значение = ПеревестиВТысячи(ТабДокумент.Области["П047905"].Значение);
		
		// Раздел 4, строка 65, графы 3, 4, 8
		ТабДокумент.Области["П046504"].Значение = СтруктураПараметровСодержаниеИРемонт.Начисление + ТабДокумент.Области["П046804"].Значение;
		ТабДокумент.Области["П046505"].Значение = СтруктураПараметровСодержаниеИРемонт.Оплата + ТабДокумент.Области["П046805"].Значение;
		ТабДокумент.Области["П046509"].Значение = СтруктураПараметровСодержаниеИРемонт.Площадь;
		
		// Раздел 2
		ТабДокумент.Области["П020704"].Значение = ТабДокумент.Области["П047104"].Значение;
		ТабДокумент.Области["П020804"].Значение = ТабДокумент.Области["П047304"].Значение;
		ТабДокумент.Области["П020904"].Значение = ТабДокумент.Области["П047204"].Значение;
		ТабДокумент.Области["П021004"].Значение = ТабДокумент.Области["П047404"].Значение;
		ТабДокумент.Области["П021104"].Значение = ТабДокумент.Области["П047504"].Значение;
		ТабДокумент.Области["П021204"].Значение = ТабДокумент.Области["П047804"].Значение + ТабДокумент.Области["П047904"].Значение;
		ТабДокумент.Области["П021504"].Значение = ТабДокумент.Области["П046604"].Значение + ТабДокумент.Области["П046704"].Значение;
		ТабДокумент.Области["П021604"].Значение = ТабДокумент.Области["П046804"].Значение;
...
Процедура ОткрытьФормуНастроек(Команда)
	ОткрытьФорму("Отчет.РегламентированныйОтчетСтатистикаФорма22ЖКХжилище.Форма.НастройкиОтчета");
КонецПроцедуры
