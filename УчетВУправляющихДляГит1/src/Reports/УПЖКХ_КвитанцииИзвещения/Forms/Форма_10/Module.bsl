...
Процедура ПриОткрытии(Отказ)
	...
	Попытка
		...
		ТекущаяФормаКвитанции = Отчет.СписокФормКвитанций.НайтиПоЗначению("10");
...
Процедура ПроцентЗаУслугиБанкаПриИзменении(Элемент)
	...
	Попытка
		...
		Если Отчет.ВыводитьПроцентЗаУслугиБанка И Не ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
			УПЖКХ_ТиповыеМетодыКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(Элементы.Результат, "НеАктуальность");
		КонецЕсли;
...
Процедура СменитьКвитанцию(Команда)
	...
	Отчет.СписокФормКвитанций.ПоказатьВыборЭлемента(ОповещениеОВыборе, "Сменить квитанцию", Отчет.СписокФормКвитанций.НайтиПоЗначению("10"));
...
Процедура СравнитьМакетСТиповым(Команда)
	...
	УПЖКХ_РаботаСДиалогамиКлиент.СравнитьТиповойМакетСПользовательским("Отчет.УПЖКХ_КвитанцииИзвещения", "ПФ_MXL_Макет_10");
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("ИсполнительРасчетов"			   , ?(ЗначениеЗаполнено(Отчет.ПолучательПлатежа), Отчет.ПолучательПлатежа, Справочники.Контрагенты.ПустаяСсылка()));
	ПараметрыОтчета.Вставить("Форма"						   , "10");
	ПараметрыОтчета.Вставить("ВидЗапроса"					   , "форма_10");
...
Процедура УправлениеФормой()
	...
	Элементы.СравнитьМакетСТиповым.Видимость = УПЖКХ_ОбщегоНазначенияКлиентСервер.ОпределитьИспользованиеПользовательскогоМакетаПечати("Отчет.УПЖКХ_КвитанцииИзвещения", "ПФ_MXL_Макет_10");
