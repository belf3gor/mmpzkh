...
Процедура СохранениеРеквизитов()
	...
	СтруктураПараметров.Вставить("КоличествоСтрокТаблицыНачислений",  Отчет.КоличествоСтрокТаблицыНачислений);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//УПЖКХ_РассылкаКвитанцийНаЭлектроннуюПочтуЛицевыхСчетовСервер.ПриСозданииНаСервере(ЭтаФорма, Элементы.ГруппаОсновнаяКоманднаяПанель,
	//															Элементы.ГруппаШапкаГиперссылки, Элементы.ГруппаРезультатПанельНастроек);
...
Процедура ЗаполнитьТаблицуОбщийСписокУслуг()
	...
	ВсеВидыУслуг = Отчеты.УПЖКХ_КвитанцииИзвещения.ПолучитьВсеВидыУслуг();
	...
	Если Отчет.ОбщийСписокУслуг.Количество() = 0 
		...
		Отчет.ОбщийСписокУслуг.Загрузить(ВсеВидыУслуг);
	ИначеЕсли ВсеВидыУслуг.Количество() <> (Отчет.ОбщийСписокУслуг.Количество() 
		...
		Для Сч = 1 По КоличествоСтр Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрВидовУслуг Из ВсеВидыУслуг Цикл
			НовСтрОбщегоСписка = Отчет.ОбщийСписокУслуг.Добавить();
			НовСтрОбщегоСписка.ВидУслуги = СтрВидовУслуг.ВидУслуги;
		КонецЦикла;
...
Процедура ПриОткрытии(Отказ)
	...
	Попытка
		...
		ТекущаяФормаКвитанции = Отчет.СписокФормКвитанций.НайтиПоЗначению("09");
...
&НаКлиенте
// Обработчик события "ПриИзменении" поля "КоличествоСтрокТаблицыНачислений".
Процедура КоличествоСтрокТаблицыНачисленийПриИзменении(Элемент)
...
Процедура СменитьКвитанцию(Команда)
	...
	Отчет.СписокФормКвитанций.ПоказатьВыборЭлемента(ОповещениеОВыборе, "Сменить квитанцию", Отчет.СписокФормКвитанций.НайтиПоЗначению("09"));
...
Процедура СравнитьМакетСТиповым(Команда)
	...
	УПЖКХ_РаботаСДиалогамиКлиент.СравнитьТиповойМакетСПользовательским("Отчет.УПЖКХ_КвитанцииИзвещения", "ПФ_MXL_Макет_09");
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("КоличествоСтрокТаблицыНачислений", Отчет.КоличествоСтрокТаблицыНачислений);
	...
	ПараметрыОтчета.Вставить("Форма"						   , "09");
	ПараметрыОтчета.Вставить("ВидЗапроса"					   , "форма_09");
	...
	// МеханизмРассылкиКвитанцийНаЭлекторонныйЯщики
	ПараметрыОтчета.Вставить("ИспользоватьРассылкуКвитанцийПоЭлектроннойПочте", Ложь); // ВРЕМЕННО СКРЫТЫЙ МЕХАНИЗМ РАССЫЛКИ КВИТАНЦИЙ!!!
	//ПараметрыОтчета.Вставить("ИспользоватьРассылкуКвитанцийПоЭлектроннойПочте", ПолучитьФункциональнуюОпцию("УПЖКХ_ИспользоватьФункционалРассылкиКвитанцийПоЭлектроннойПочте"));
	ПараметрыОтчета.Вставить("ДанныеПоКвитанциям", УПЖКХ_РассылкаКвитанцийНаЭлектроннуюПочтуЛицевыхСчетовСервер.ПолучитьПустуюТаблицуДляДанныхПоКвитанциям());
	// Конец МеханизмРассылкиКвитанцийНаЭлекторонныйЯщики
...
Процедура УправлениеФормой()
	...
	Элементы.СравнитьМакетСТиповым.Видимость = УПЖКХ_ОбщегоНазначенияКлиентСервер.ОпределитьИспользованиеПользовательскогоМакетаПечати("Отчет.УПЖКХ_КвитанцииИзвещения", "ПФ_MXL_Макет_09");
...
Процедура ЗагрузитьПодготовленныеДанныеНаСервере()
	...
	// ВРЕМЕННО СКРЫТЫЙ МЕХАНИЗМ РАССЫЛКИ КВИТАНЦИЙ!!!
	//// МеханизмРассылкиКвитанцийНаЭлекторонныйЯщики
	//Если ПолучитьФункциональнуюОпцию("УПЖКХ_ИспользоватьФункционалРассылкиКвитанцийПоЭлектроннойПочте") Тогда
