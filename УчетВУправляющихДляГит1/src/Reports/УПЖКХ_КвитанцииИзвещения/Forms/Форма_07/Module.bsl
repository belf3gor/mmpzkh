// Таблица "Расшифровка начислений по индивидуальным ПУ" выводит перечень назначенных счетчиков, 
// по которым не были переданы показания за текущий месяц. В данной таблице жильцы самостоятельно 
// указывают текущие показания, рассчитывают расход и сумму к оплате. Если показания были переданы 
// до формирования квитанции, то таблица не выводится, а начисления указываются в общей таблице начислений.
...
&НаСервере
// Получает список объектов печати с учетом настройки "Выводить по льготам".
//
Процедура ПолучитьСписокЛицевыхСУчетомЛьгот(ПараметрыОтчета, ТаблицаОбъектов)
	...
	Если НЕ ПараметрыОтчета.ВыводитьПоЛьготам = "всех" Тогда
		...
		Запрос.УстановитьПараметр("СписокЛС", УПЖКХ_ПечатьКвитанцийСервер.ПолучитьСписокОбъектов(ТаблицаОбъектов.Скопировать(), "ЛицевойСчет"));
		...
		МассивСтрокЛС = Новый Массив;
		Для Каждого СтрокаОбъектов ИЗ ТаблицаОбъектов Цикл
			Если НЕ СписокЛС.НайтиПоЗначению(СтрокаОбъектов.ЛицевойСчет) = Неопределено Тогда
				МассивСтрокЛС.Добавить(СтрокаОбъектов);
			КонецЕсли;
		КонецЦикла;
		
		ТаблицаОбъектов = ТаблицаОбъектов.Скопировать(МассивСтрокЛС);
...
Процедура СохранениеРеквизитов()
	...
	СтруктураПараметров.Вставить("ВыводитьЗависимыеУслуги",         Отчет.ВыводитьЗависимыеУслуги);
...
Процедура ПриОткрытии(Отказ)
	...
	Попытка
		...
		ТекущаяФормаКвитанции = Отчет.СписокФормКвитанций.НайтиПоЗначению("07");
...
&НаКлиенте
// Обработчик события "ПриИзменении" поля "СуммаОплаты".
...
&НаКлиенте
// Обработчик события "ПриИзменении" поля "ВыводитьПоЛьготам".
Процедура ВыводитьПоЛьготамПриИзменении(Элемент)
...
&НаКлиенте
// Обработчик события "ПриИзменении" поля "ВыводитьЗависимыеУслуги".
Процедура ВыводитьЗависимыеУслугиПриИзменении(Элемент)
...
Процедура СменитьКвитанцию(Команда)
	...
	Отчет.СписокФормКвитанций.ПоказатьВыборЭлемента(ОповещениеОВыборе, "Сменить квитанцию", Отчет.СписокФормКвитанций.НайтиПоЗначению("07"));
...
Процедура СравнитьМакетСТиповым(Команда)
	...
	УПЖКХ_РаботаСДиалогамиКлиент.СравнитьТиповойМакетСПользовательским("Отчет.УПЖКХ_КвитанцииИзвещения", "ПФ_MXL_Макет_07");
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("ВыводитьЗависимыеУслуги"		   , Отчет.ВыводитьЗависимыеУслуги);
	ПараметрыОтчета.Вставить("ВыводитьПоЛьготам"			   , ВыводитьПоЛьготам);
	ПараметрыОтчета.Вставить("СуммаОплаты"					   , СуммаОплаты);
	ПараметрыОтчета.Вставить("КассовыйСборБанка"			   , 3);
	ПараметрыОтчета.Вставить("ИсполнительРасчетов"			   , Отчет.ПолучательПлатежа);
	ПараметрыОтчета.Вставить("Форма"						   , "07");
	ПараметрыОтчета.Вставить("ВидЗапроса"					   , "форма_07");
	...
	ПолучитьСписокЛицевыхСУчетомЛьгот(ПараметрыОтчета, ТаблицаОбъектов);
...
Процедура УправлениеФормой()
	...
	Элементы.СравнитьМакетСТиповым.Видимость = УПЖКХ_ОбщегоНазначенияКлиентСервер.ОпределитьИспользованиеПользовательскогоМакетаПечати("Отчет.УПЖКХ_КвитанцииИзвещения", "ПФ_MXL_Макет_07");
