...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("ВыводитьРегистратор"              , Отчет.ВыводитьРегистратор);
	...
	ПараметрыОтчета.Вставить("Периодичность"                     , 
		? (ВидПериода = Перечисления.ДоступныеПериодыОтчета.Год, 12, 
		? (ВидПериода = Перечисления.ДоступныеПериодыОтчета.Полугодие, 11,
		? (ВидПериода = Перечисления.ДоступныеПериодыОтчета.Квартал, 10,
		9))));
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru = 'План-фактный анализ по затратам ТСЖ за период '")
...
#Область ОбработчикиСобытийТабличногоПоля_
...
&НаКлиенте
// Обработчик события "Нажатие" поля "НастройкаСчетовУчетаРасчетов".
Процедура НастройкаСчетовУчетаРасчетовНажатие(Элемент)
	
	ФормаСпискаСчетов = ПолучитьФорму("Отчет.КВП_ПланФактныйАнализРасходовТСЖ.Форма.ФормаСпискаСчетов",
									  Новый Структура("СписокСчетов", Отчет.Счет), ЭтаФорма);
	ФормаСпискаСчетов.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ОбработатьРезультатЗакрытияФормыПланФактногоАнализа", ЭтаФорма);
	ФормаСпискаСчетов.Открыть();
	
КонецПроцедуры // НастройкаСчетовУчетаРасчетовНажатие()

&НаКлиенте
// Обработчик результата закрытия формы, вызванного в процедуре "НастройкаСчетовУчетаРасчетовНажатие()".
Процедура ОбработатьРезультатЗакрытияФормыПланФактногоАнализа(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	
	// Счета учета расчетов с покупателями обновляем только, если форма списка счетов была закрыта по кнопке ОК.
	Если ТипЗнч(РезультатЗакрытия) = Тип("СписокЗначений") Тогда
		Отчет.Счет = РезультатЗакрытия;
	КонецЕсли;
	
КонецПроцедуры // ОбработатьРезультатЗакрытияФормыПланФактногоАнализа()

#КонецОбласти

#Область ОбработчикиСобытийГруппы_Группировка
...
#Область ОбработчикиСобытийГруппы_Отборы
...
#Область ОбработчикиСобытийГруппы_ДополнительныеПоля
...
#Область ОбработчикиСобытийГруппы_Сортировка
...
#Область ОбработчикиСобытийГруппы_Оформление
