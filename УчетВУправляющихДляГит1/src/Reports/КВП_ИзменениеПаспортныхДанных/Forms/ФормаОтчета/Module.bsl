...
Процедура ОбработатьИзменениеОперации()
	...
	УПЖКХ_ТиповыеМетодыКлиентСервер.ИзменитьЭлементОтбораГруппыСписка(Отчет.КомпоновщикНастроек.Настройки.Отбор,
																		"Операция");
	
	Если ЗначениеЗаполнено(Отчет.Операция) Тогда
		УПЖКХ_ТиповыеМетодыКлиентСервер.ДобавитьОтбор(Отчет.КомпоновщикНастроек, "Операция", Отчет.Операция);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Операция.СписокВыбора.Добавить("Регистрация", "Регистрация");
	Элементы.Операция.СписокВыбора.Добавить("Снятие с учета", "Снятие с учета");
	ВыборкаПричины = Справочники.КВП_ПричиныИзмененияПаспортныхДанных.Выбрать();
	Пока ВыборкаПричины.Следующий() Цикл
		Элементы.Операция.СписокВыбора.Добавить(ВыборкаПричины.Ссылка, ВыборкаПричины.Наименование);
	КонецЦикла;
...
&НаКлиенте
// Обработчик события "ПриИзменении" поля "Операция".
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("Операция"                         , Отчет.Операция);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Изменение паспортных данных "
...
Функция ПолучитьПараметрыВыбораЗначенияОтбора()
	...
	СписокПараметров.Вставить("Операция", Отчет.Операция);
