...
Процедура ПриОткрытии(Отказ)
	...
	// Устанавливаем отбор по лицевому счету.
	ПолеКомпоновкиЛС = Новый ПолеКомпоновкиДанных("ЛицевойСчет");
	Если Не ЛицевойСчет.Пустая() Тогда
		ЭлементыДляУдаления = Новый Массив;
		КоллекцияЭлементовОтбора = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы;
		Для Каждого ТекЭлемент Из КоллекцияЭлементовОтбора Цикл
			Если ТекЭлемент.ЛевоеЗначение = ПолеКомпоновкиЛС Тогда
				ЭлементыДляУдаления.Добавить(ТекЭлемент);
			КонецЕсли;
		КонецЦикла;
		
		Для Каждого ТекЭлемент Из ЭлементыДляУдаления Цикл
			КоллекцияЭлементовОтбора.Удалить(ТекЭлемент);
		КонецЦикла;
		
		УПЖКХ_ТиповыеМетодыКлиентСервер.ДобавитьОтбор(Отчет.КомпоновщикНастроек, "ЛицевойСчет", ЛицевойСчет,ВидСравненияКомпоновкиДанных.ВИерархии,, Истина);
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	УПЖКХ_ТиповыеМетодыКлиент.ПередЗакрытием(ЭтаФорма, Отказ, СтандартнаяОбработка);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Отчет по начислениям и долгам "
