...
Функция ПолучитьТаблицуПараметровПроведения(ИсходнаяТаблица, СписокКолонок) Экспорт
	...
	Иначе
		Если СтрНайти(СписокКолонок,"ЗданиеКапРем") <> 0 и ИсходнаяТаблица.Колонки.найти("ЗданиеКапРем") = Неопределено   тогда
		ИсходнаяТаблица.Колонки.Добавить("ЗданиеКапРем",Новый ОписаниеТипов("СправочникСсылка.КВП_Здания"));   //временный костыль, переделать.
		КонецЕсли;
...
Процедура УстановитьЗначениеПоУмолчанию(Настройка, Значение, ИмяПользователяИБ = Неопределено) Экспорт
	...
	Если ВРег(Настройка) = ВРег("ОсновнаяОрганизация")
		...
		// Квартплата +
		ИЛИ ВРег(Настройка) = ВРег("НеИспользоватьФункционалЖКХ")
		ИЛИ ВРег(Настройка) = ВРег("ОсновнаяСтавкаНДС")
		ИЛИ ВРег(Настройка) = ВРег("ОсновнойВидОперацииРегистрацияОплаты") Тогда
		// Квартплата -
