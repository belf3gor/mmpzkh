...
Функция ПериодУплатыНалогаВыбираетПользователь(ВидНалога, ПрименяетсяУСНДоходы) Экспорт
	
	// При уплате некоторых налогов/взносов пользователь может изменять период, за который осуществляется платеж.
	// Доступность изменения периода зависит от конкретного налога и применяемой системы налогообложения.
