...
Процедура ПриНачалеРаботыСистемы(Знач ОповещениеЗавершения = Неопределено, НепрерывноеВыполнение = Истина) Экспорт
	...
	Если СЗК_Параметры.ВыполнятьОсвобождениеЛицензийСПомощьюРегламентногоЗадания() Тогда
		СЗК_МодульЗащиты.АктуализироватьНастройкиОбновленияСостоянийЛицензийСеансов();
	КонецЕсли;
	
	// Проверку выполняем, если не выполняется интерактивная обработка данных.
	Если СЗК_МодульЗащиты.ЗащищаемыйБлокДоступен()
	   И Не СЗК_МодульЗащиты.ЛицензионныйКлючКорректен(, Ложь) Тогда
		
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ПоказатьОшибкуПриЗапуске", Истина);
		
		ОткрытьФорму("ОбщаяФорма.СЗК_СообщениеОбОтсутствииЛицензии", ПараметрыОткрытия);
		
	КонецЕсли;
	
	// Подключение обработчика мониторинга состояния СЛК при запуске программы.
	Если Не СЗК_МодульЗащиты.ЭтоNFRВерсия() Тогда
		ПодключитьОбработчикОжидания("МониторингСостоянияСЛК", 30, Истина); // Через 30 секунд после запуска клиента.
	КонецЕсли;
...
Процедура ДействияПередНачаломРаботыСистемыВМодуляхИнтеграционнойПроцедуры(Неопределен, Контекст) Экспорт
	...
	Попытка
		...
		Иначе
			...
			// Изменим порядок выполнения обработчиков:
			// - перед выполнением обновления информационной базы БП проверим лицензию.
			// - после выполнения обновления информационной базы БП выполним обновление ИБ ЖКХ.
			Если ОписаниеМодуля.Номер = 2 Тогда
				ТекущийМодуль.ПередНачаломРаботыСистемы2(Параметры);
			ИначеЕсли ОписаниеМодуля.Номер = 3 Тогда
				ТекущийМодуль.ПередНачаломРаботыСистемы3(Параметры);
			ИначеЕсли ОписаниеМодуля.Номер = 4 Тогда
				ТекущийМодуль.ПроверитьНаличиеКопииПередЗапускомКритичногоОбновления(Параметры);
			ИначеЕсли ОписаниеМодуля.Номер = 5 Тогда
				ТекущийМодуль.ПроверитьНаличиеЛицензииЖКХ(Параметры);
			ИначеЕсли ОписаниеМодуля.Номер = 6 Тогда
				ТекущийМодуль.ПередНачаломРаботыСистемы4(Параметры);
			ИначеЕсли ОписаниеМодуля.Номер = 7 Тогда
				ТекущийМодуль.ПередНачаломРаботыСистемы5(Параметры);
			КонецЕсли;
