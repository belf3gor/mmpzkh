	...
	Если Дескриптор.DataType = "ЭталонОбластиДанных" Тогда
		...
		Для каждого Характеристика Из Дескриптор.Properties.Property Цикл
			Если Не УсловиеИмениКонфигурации Тогда
				...
				УсловиеИмениКонфигурации = Характеристика.Code = "ИмяКонфигурации" И Характеристика.Value = КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя);
				// Квартплата -
			КонецЕсли;
			Если Не УсловиеВерсииКонфигурации Тогда
				...
				// Квартплата +
					...
					И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Характеристика.Value, КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия)) >= 0;
				// Квартплата -
			КонецЕсли;
		КонецЦикла;
...
Функция ЗагрузитьОбластьИзПоставляемыхДанных(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, Знач Вариант, СообщениеОФатальнойОшибке)
	...
	Фильтр.Добавить(Новый Структура("Код, Значение", "ИмяКонфигурации", КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя)));
	Фильтр.Добавить(Новый Структура("Код, Значение", "ВерсияКонфигурации", КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия)));
	// Квартплата -
