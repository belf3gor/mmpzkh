...
Процедура ЗаблокироватьДополнительныеОтчетыИОбработкиДляОбновления() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Запрос.УстановитьПараметр("Версия", КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия));
		// Квартплата -
...
Функция ПроверитьСовместимостьПоставляемойОбработки(Знач ТаблицаСовместимости)
	...
	Для Каждого ДекларацияСовместимости Из ТаблицаСовместимости Цикл
		...
		Если ПустаяСтрока(ДекларацияСовместимости.VersionNumber) Тогда
			...
			Если ДекларацияСовместимости.ConfigarationName = КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя) Тогда
			// Квартплата -
				Возврат Истина;
			КонецЕсли;
		...
		Иначе
			...
			Если ДекларацияСовместимости.ConfigarationName = КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя) И ДекларацияСовместимости.VersionNumber = КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия) Тогда
			// Квартплата -
				Возврат Истина;
			КонецЕсли;
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	Если ПоставляемаяОбработка.ВерсияGUID <> Новый УникальныйИдентификатор(ОписаниеПоставляемойОбработки.Версия) Тогда
		...
		Для Каждого ИнформацияОСовместимости Из ТаблицаСовместимости Цикл
			...
			Если ИнформацияОСовместимости.ConfigarationName = КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя) Тогда
			// Квартплата -
