...
Процедура ЗаполнитьАналитикуСчетаРасчетов(Проводка, СтрокаТаблицы, ТипАналитики, ОтражатьВНалоговомУчете, СуммаВР = 0, Сторно = Ложь)
	...
	КонецЕсли;
		...
		//Позолотина ++
	Если Счет = ПланыСчетов.Хозрасчетный.ЦелевыеПоступленияСобственников и ТипАналитики = "Дт"  тогда
		если Проводка.Содержание = "Корректировка реализации" тогда
			  БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],3,         СтрокаТаблицы.КорСубконто3.Здание);
			  иначе	Если Значениезаполнено(СтрокаТаблицы.зданиеКапРем) тогда
				     	  БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],3,         СтрокаТаблицы.ЗданиеКапРем);
			   		иначе БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],3,         СтрокаТаблицы.ДокументРасчетовСКомитентом.Здание);
					конецесли;   
			   конецесли;
	конецесли;
	Если Счет = ПланыСчетов.Хозрасчетный.РасчетыПоПричитающимсяДивидендам и ТипАналитики = "Дт"  тогда
		если Проводка.Содержание = "Корректировка реализации" тогда
...
Процедура  ЗаполнитьАналитикуСчетаДохода(Проводка, СтрокаТаблицы, ТипАналитики, ОтражатьВНалоговомУчете, СуммаВР = 0)
	...
	КонецЕсли;
	  	//Позолотина++
	Если Счет = ПланыСчетов.Хозрасчетный.ЦелевыеПоступленияСобственников и ТипАналитики = "Кт"  тогда
		если Проводка.Содержание = "Корректировка реализации" тогда
			  БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],3,         СтрокаТаблицы.КорСубконто3.Здание);
		  иначе Если Значениезаполнено(СтрокаТаблицы.зданиеКапРем) тогда
				     	  БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],3,         СтрокаТаблицы.ЗданиеКапРем);
	...
	Если ОтражатьВНалоговомУчете И СвойстваСчета.НалоговыйУчет Тогда
			//Позолотина++
		Если не Счет = ПланыСчетов.Хозрасчетный.РезервыПредстоящихРасходов тогда
...
Процедура ЗаполнитьАналитикуСчетаРасчетовСКомитентом(Проводка, СтрокаТаблицы, ТипАналитики, ДокументРасчетов, ОтражатьВНалоговомУчете)
	...
	Если Счет = ПланыСчетов.Хозрасчетный.ЦелевыеПоступленияСобственников и ТипАналитики = "Дт"  тогда
		если Проводка.Содержание = "Корректировка реализации" тогда
			  БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],2,         СтрокаТаблицы.КорСубконто3.Здание);
			  иначе	Если Значениезаполнено(СтрокаТаблицы.зданиеКапРем) тогда
				     	  БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],2,         СтрокаТаблицы.ЗданиеКапРем);
			   		иначе БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики],2,         СтрокаТаблицы.ДокументРасчетовСКомитентом.Здание);
					конецесли;   
			   конецесли;
	конецесли;
	Если Счет = ПланыСчетов.Хозрасчетный.РасчетыПоПричитающимсяДивидендам и ТипАналитики = "Дт"  тогда
