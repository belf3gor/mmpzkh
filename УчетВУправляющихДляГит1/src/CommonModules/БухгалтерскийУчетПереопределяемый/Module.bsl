...
Функция ВидыКонтактнойИнформацииКомпании(ТипКомпании)
	...
	ВидыКонтактнойИнформацииКомпании = Новый Структура("ЮридическийАдрес, ФактическийАдрес, ПочтовыйАдрес, Телефоны, ТелефонЮридическогоАдреса, Факс, Email");
...
Функция ПолучитьЗначениеПоУмолчанию(Настройка, ИмяПользователяИБ = Неопределено) Экспорт
	...
	// Квартплата +
	ИначеЕсли НастройкаВРег = ВРег("НеИспользоватьФункционалЖКХ") Тогда
		ПустоеЗначение = Ложь;
	ИначеЕсли НастройкаВРег = ВРег("ОсновнаяСтавкаНДС") Тогда
		ПустоеЗначение = Перечисления.СтавкиНДС.БезНДС;
		ИмяОбъекта = "Перечисление.СтавкиНДС";
		НастройкаТипаСсылка = Истина;
	ИначеЕсли НастройкаВРег = ВРег("ОсновнойВидОперацииРегистрацияОплаты") Тогда
		ПустоеЗначение = Перечисления.КВП_ВидыОперацийРегистрацииОплаты.ПустаяСсылка();
		ИмяОбъекта = "Перечисление.КВП_ВидыОперацийРегистрацииОплаты";
		НастройкаТипаСсылка = Истина;
	// Квартплата -
