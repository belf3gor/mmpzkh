...
Функция ПолучитьСведенияОПоказателяхОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета = Неопределено) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПрибыль" Тогда
		...
		Иначе
			...
			// ... четвертая - модулем УставныйКапитал
...
Функция ИДКонфигурации() Экспорт
	...
	МетаданныеИмя = КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя);
	// Квартплата -
...
Функция ПолучитьСписокРегламентированныхОтчетов() Экспорт
	...
	// Квартплата +
	СписокРегламентированныхОтчетов.Добавить("УПЖКХ_РегламентированныйОтчетСтатистикаФорма1СведенияОЖилФонде");
	СписокРегламентированныхОтчетов.Добавить("УПЖКХ_РегламентированныйОтчетСтатистикаФормаN1_ПУ_ЖКХ");
	// Квартплата -
...
Процедура ЗаписьОбъектовРегламентированнойОтчетности(Ссылка, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет")
		...
		Если РеквизитыОбъекта.ПометкаУдаления
			...
			РегистрыСведений.ВыполнениеРегламентныхОперацийНДС.СброситьФактВыполненияРегламетнойОперации(
