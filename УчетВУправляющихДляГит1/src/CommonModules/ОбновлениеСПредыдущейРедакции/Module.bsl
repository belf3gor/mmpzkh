...
Функция ЭтоПереходСРедакции20() Экспорт
	...
	Если ИнформационнаяБазаОбновленаС20() И ЭтоВерсияБП2
		// Квартплата +
		//И ОбновлениеИнформационнойБазы.ВерсияИБ()="0.0.0.0" Тогда
		И ОбновлениеИнформационнойБазы.ВерсияИБ(КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя))="0.0.0.0" Тогда
		// Квартплата -
...
Процедура ОчиститьВерисиюПодсистемыБСППриПереходеС2БП() Экспорт
	...
	Если ЭтоПереходСРедакции20() Тогда
		...
		ВерсииПодсистем.ИмяПодсистемы = КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя);
		// Квартплата -
		ВерсииПодсистем.Активность = Истина;
...
Процедура ПереносДопСвойствПриОбновленииС20()
	...
	// Квартплата +
	СоответствиеТипов.Вставить(Тип("СправочникСсылка.УПЖКХ_Жильцы")                  , "СправочникСсылка.УПЖКХ_Жильцы");
	// Квартплата -
...
Процедура ДокументыФизическихЛицКонвертация(СоответствиеКонвертированныхОбъектов)
	...
	// Квартплата +
	|	УдалитьПаспортныеДанныеФизЛиц.ПричинаИзменения,
	|	УдалитьПаспортныеДанныеФизЛиц.ДокументРегистратор,
	// Квартплата -
	...
	Пока Выборка.Следующий() Цикл
		...
		// Квартплата +
		ЗаписьНабора.ПричинаИзменения	 = Выборка.ПричинаИзменения;
		ЗаписьНабора.ДокументРегистратор = Выборка.ДокументРегистратор;
		// Квартплата -
