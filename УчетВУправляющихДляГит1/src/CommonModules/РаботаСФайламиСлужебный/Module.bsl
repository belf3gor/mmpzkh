...
Функция ИменаСправочниковХраненияФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь) Экспорт
	...
	ИмяСтандартногоОсновногоСправочника = ПолучитьИмяСправочникаПрисоединенныхФайлов(МетаданныеВладельца.Имя);
	
	Если СтрНайти(ИмяСтандартногоОсновногоСправочника, "УПЖКХ_") = 0 Тогда
		ИмяСтандартногоОсновногоСправочника = МетаданныеВладельца.Имя
		+ ?(СтрЗаканчиваетсяНа(МетаданныеВладельца.Имя, "ПрисоединенныеФайлы"), "", "ПрисоединенныеФайлы");
	КонецЕсли;
...
#Область МетодыПодсистемыКвартплаты

// Квартплата +

// Так как для наших объектов используется один справочник,
// то будем использовать данную функцию, которая возвращает имя справочника.
Функция ПолучитьИмяСправочникаПрисоединенныхФайлов(Знач ИмяОбъекта) Экспорт
	
	// Если имя содержит префикс наших объектов, то в качестве справочника
	// хранения файлов используется справочник "УПЖКХ_ПрисоединенныеФайлы".
	Если Найти(ИмяОбъекта, "КВП") > 0 Или Найти(ИмяОбъекта, "УПЖКХ") > 0 Или ИмяОбъекта = "ФизическиеЛица" Или ИмяОбъекта = "Контрагенты" Тогда
		ИмяОбъекта = "УПЖКХ_";
	КонецЕсли;
	
	Возврат ИмяОбъекта + "ПрисоединенныеФайлы";
