...
// Закрывает забалансовые счета, на которых в течение налогового периода накапливаются суммы расходов,
// не принятых для целей налогообложения.
// Проводки в Дт этих счетов формируются в ОтразитьДоходыРасходыНеУчитываемыеВНалоговомУчете регистра бухгалтерии.
//
// Параметры:
...
//                проводки по Дт счета формируются в разрезе филиалов, а проводки по Кт - по головной организации.
//
...
Процедура ЗаполнитьОтчетПрибыльФормаОтчета2015Кв1(Контейнер, ТаблицаРасшифровки, ИДРедакцииОтчета, КонтекстРасчета) Экспорт
	...
	// Строка 131 - Сумма начисленной амортизации, за отчетный (налоговый) период - всего
	//++ Позолотина добавим счет 01.01 в массив СчетаОС для того, чтобы попадало принятие ОС к учету с ручной корректировкой проводок
	пМассив = новый Массив; 
	ИсхМассив = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПС.АмортизацияОсновныхСредств);
	Для каждого ЭлементИсхМассива из ИсхМассив Цикл
		пМассив.Добавить(ЭлементИсхМассива);
	КонецЦикла;
	пМассив.Добавить(ПланыСчетов.Хозрасчетный.ОСвОрганизации);
	фМассив_ОС = новый ФиксированныйМассив(пМассив); // создаем новый фиксированный массив
	Запрос.УстановитьПараметр("СчетаОС", фМассив_ОС);
	//--
	...
	Если Месяц(КонтекстРасчета.КонецПериода) = 3 Или Месяц(КонтекстРасчета.КонецПериода) = 12 Тогда
		...
		Пока СчетчикГодов > ПоследнийГод - 12 Цикл        //++ Позолотина добавила  вместо 10 лет 12  24_03_2019
			СтрокаТаблицаУбытков = ТаблицаУбытков.Добавить();
			СтрокаТаблицаУбытков.ГодУбытка = СчетчикГодов;
			СчетчикГодов = СчетчикГодов - 1;
		КонецЦикла;
		...
		Для Каждого УбытокПрошлыхЛет Из УбыткиПрошлыхЛет Цикл
			...
			КонецЕсли;
			СтрокаТаблицаУбытков.ГодУбыткаПредставление = СтрЗаменить(Строка(СтрокаТаблицаУбытков.ГодУбытка)," ", "");
			//СтрокаТаблицаУбытков.Сумма   = УбытокПрошлыхЛет.СуммаНачальныйОстатокДт; //++ Позолотина 24_03_2019
			СтрокаТаблицаУбытков.Сумма   = УбытокПрошлыхЛет.СуммаКонечныйОстатокДт;
			СтрокаТаблицаУбытков.Остаток = УбытокПрошлыхЛет.СуммаКонечныйОстатокДт;
			СтрокаТаблицаУбытков.РБП     = УбытокПрошлыхЛет.Субконто1;
		...
		Для Каждого СтрокаТаблицаУбытков Из ТаблицаУбытков Цикл
		Если не СтрокаТаблицаУбытков.Остаток = 0 или не СтрокаТаблицаУбытков.Сумма = 0 тогда // ++ Позолотина чтобы не брала пустые строки 24_03_2019
			ПредставлениеПоказателя = Формат(НомерПоказателя, "ЧЦ=2; ЧВН=");
...
Процедура ОтразитьПрибылиУбыткиПоВозвратамТоваровПрошлыхЛет(Проводки, ПрибылиУбытки, Период, Организация)
	...
	СтатьяПрибылиУбытки = Справочники.ПрочиеДоходыИРасходы.СтатьяВозвратТоваровПрошлыхЛет();
