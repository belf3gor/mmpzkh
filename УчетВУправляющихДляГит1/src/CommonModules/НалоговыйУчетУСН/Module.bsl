...
Процедура ЗаполнитьСписокВыбораПорядкаОтраженияАвансовДляТЧ(ПолеОтражениеАванса, СоответствиеОтражениеАванса = Неопределено, Объект) Экспорт
	...
	// или явное указание режима налогообложения в документе.
	Если УчетУСН.ЕстьАвансыПокупателей(ВидОперации)
...
Функция ДеятельностьНаПатентеПоУмолчанию(Организация, Знач Дата) Экспорт
	...
	Если УчетнаяПолитика.ПрименяетсяУСНПатент(Организация, Дата) Тогда
		...
		ИначеЕсли УчетнаяПолитика.ПрименяетсяОсобыйПорядокНалогообложения(Организация, Дата) Тогда
			...
			СписокПатентов = Справочники.Патенты.ПатентыОрганизации(Организация, Дата);
...
Функция ПорядокОтраженияАвансовПоУмолчанию(Организация, Знач Дата)
	...
	ИначеЕсли УчетнаяПолитика.ПрименяетсяОсобыйПорядокНалогообложения(Организация, Дата) Тогда
		...
		Если УчетнаяПолитика.ПрименяетсяУСНПатент(Организация, Дата) И НЕ УчетнаяПолитика.ПлательщикЕНВД(Организация, Дата) Тогда
			...
			МассивПатентов = Справочники.Патенты.ПатентыОрганизации(Организация, Дата);
	...
	Если (ВидОперации = "Поступление")
		...
		Если ТипЗнч(Договор) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			...
			Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
				...
				СуммаВзаиморасчетов = СуммаВзаиморасчетов - РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(
		...
		Если ДвиженияРегистров <> Неопределено Тогда
			...
			Если (ДвиженияРегистров.КУДиР.Количество() > 0)
				...
				Для каждого Расход Из ДеревоРасходов Цикл
					...
					// Квартплата +
					// Необходимо выполнить корректировку значений граф Графа4 и Графа5 записи КУДиР
					// в соответствии с настройками учетной политики ЖКХ:
					УПЖКХ_НалоговыйУчетУСН.СкорректироватьЗначенияГрафКУДиРВСоответствииСНастройкамиУПЖКХ(СтруктураШапкиДокумента.Объект, ЗаписьКУДиР);
					// Квартплата -
...
Процедура РасчетыСКонтрагентамиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, ВидОперации = "Оплата", ТаблицаНДСПродажи = Неопределено)
	...
	// Квартплата +
	// Тип "Расчетного документа" должен быть прописан в типах измерения "РасчетныйДокумент" или "Партия" регистра "РасходыПриУСН".
	// Если такого типа там нет, то документ в массив условий не добавлять, иначе будет ошибка при установке блокировки.
	// Вставка нужна для случая, когда в качестве документа расчета появляется документ подсистемы ЖКХ,
	// а такой документ не включен в состав типов измерений регистра "РасходыПриУСН".
	Если ТипЗнч(МассивУсловия) = Тип("Массив") Тогда
		врМассивУсловия = Новый Массив;
		Для Каждого ТекущийДокумент Из МассивУсловия Цикл
			Если Метаданные.РегистрыНакопления.РасходыПриУСН.Измерения[УсловиеПоПолюРасходы].Тип.СодержитТип(ТипЗнч(ТекущийДокумент)) Тогда
				врМассивУсловия.Добавить(ТекущийДокумент);
			КонецЕсли;
		КонецЦикла;
		МассивУсловия = врМассивУсловия;
	КонецЕсли;
	// Квартплата -
