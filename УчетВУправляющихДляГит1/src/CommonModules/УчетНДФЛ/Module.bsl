...
Функция РеестрПеречисленныхСуммНалога(РеквизитыПлатежа) Экспорт 
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УплатаНДФЛНалоговымиАгентамиКРаспределениюОбороты.Организация КАК Организация,
	|	УплатаНДФЛНалоговымиАгентамиКРаспределениюОбороты.МесяцНалоговогоПериода КАК МесяцНалоговогоПериода,
	|	УплатаНДФЛНалоговымиАгентамиКРаспределениюОбороты.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	УплатаНДФЛНалоговымиАгентамиКРаспределениюОбороты.РеквизитыПлатежногоПоручения КАК РеквизитыПлатежногоПоручения,
	|	УплатаНДФЛНалоговымиАгентамиКРаспределениюОбороты.ДатаПлатежа КАК ДатаПлатежа
	...
	|	ДанныеДокументов.Бухгалтер КАК Бухгалтер
	...
	|	ДанныеДокументов.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА УплатаПоФизическимЛицам.МесяцНалоговогоПериода < ДАТАВРЕМЯ(2015, 1, 1, 0, 0, 0)
	|			ТОГДА УплатаПоФизическимЛицам.МесяцНалоговогоПериода
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|	КОНЕЦ КАК МесяцНалоговогоПериода,
	|	ДанныеДокументов.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ДанныеДокументов.РеквизитыПлатежногоПоручения КАК РеквизитыПлатежногоПоручения,
	|	ДанныеДокументов.ДатаПлатежа КАК ДатаПлатежа,
	|	УплатаПоФизическимЛицам.ФизическоеЛицо КАК ФизическоеЛицо,
		...
		Пока Выборка.СледующийПоЗначениюПоля("РеквизитыПлатежногоПоручения") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("РегистрацияВНалоговомОргане") Цикл
				Пока Выборка.СледующийПоЗначениюПоля("МесяцНалоговогоПериода") Цикл
					...
					Если ЗначениеЗаполнено(Выборка.МесяцНалоговогоПериода) Тогда
						ШаблонСтроки = НСтр("ru = 'Месяц налогового периода: %1 г.'");
						МесяцНалоговогоПериода = Формат(Выборка.МесяцНалоговогоПериода, "ДФ='ММММ гггг'");
						ОбластьМакетаШапка.Параметры.ЗаголовокМесяцНалоговогоПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтроки, МесяцНалоговогоПериода);
					КонецЕсли;
