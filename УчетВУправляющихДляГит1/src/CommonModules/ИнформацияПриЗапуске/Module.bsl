...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	// квартплата+
	Параметры.Вставить("ЖКХ_ИнформацияПриЗапуске", Новый ФиксированнаяСтруктура(ГлобальныеНастройкиЖКХ()));
	// квартплата-
...
Функция ГлобальныеНастройки()
	...
	// Оповещение о необходимости проверки формул в нормах потребления услуг.
	Настройки.Вставить("ПоказатьОповещениеОПроверкеФормул", Ложь);
	Если УПЖКХ_ОбщегоНазначенияСервер.ОпределитьНаличиеПравНаРедактированиеОбъектовЖКХ() Тогда
		ПараметрыОповещения = Справочники.КВП_НормыПотребленияУслуг.ПолучитьПараметрыОповещенияОПроверкеФормул();
		Если ПараметрыОповещения.Показывать = Истина Тогда
			ПоказатьОповещениеОПроверкеФормул = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(ПараметрыОповещения.Тема, "Показывать", Истина);
			Настройки.ПоказатьОповещениеОПроверкеФормул = ПоказатьОповещениеОПроверкеФормул;
		КонецЕсли;
	КонецЕсли;
	
	// Оповещение о центре мониторинга.
	Настройки.Вставить("ПоказатьОповещениеОЦентреМониторинга", Ложь);
	ПараметрыОповещения = Обработки.УПЖКХ_ВзаимодействиеСПользователями.ПолучитьПараметрыОповещенияОЦентреМониторинга();
	Если ПараметрыОповещения.Показывать = Истина Тогда
		ПоказатьОповещениеОЦентреМониторинга = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(ПараметрыОповещения.Тема, "Показывать", Истина);
		Настройки.ПоказатьОповещениеОЦентреМониторинга = ПоказатьОповещениеОЦентреМониторинга;
	КонецЕсли;
...
// Глобальные настройки отображения информации ЖКХ.
Функция ГлобальныеНастройкиЖКХ()
	
	Настройки = Новый Структура;
	Настройки.Вставить("Показывать", Ложь);
	
	Если Метаданные.Обработки.КВП_ДополнительнаяИнформация.Макеты.Количество() > 1 Тогда
		Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
			Настройки.Показывать = Истина;
		Иначе
			Настройки.Показывать = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЖКХ_ИнформацияПриЗапуске", "Показывать", Истина);
		КонецЕсли;
	КонецЕсли;
