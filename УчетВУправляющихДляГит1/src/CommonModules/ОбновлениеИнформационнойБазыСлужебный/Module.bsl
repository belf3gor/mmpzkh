...
Функция ВерсияИБ(Знач ИдентификаторБиблиотеки, Знач ПолучитьВерсиюОбщихДанных = Ложь) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		// Квартплата +
		// Для подсистемы ЖКХ не нужно включать поддержку механизмов обновления
		// из старых версий БСП, т.к. в помеченном на удаление регистре сведений
		// может храниться информация о неактуальной версии подсистемы.
		
		//Если ПустаяСтрока(Результат) Тогда
		Если ПустаяСтрока(Результат)
		   И ИдентификаторБиблиотеки <> Метаданные.Имя
		   И ИдентификаторБиблиотеки <> УПЖКХ_ИнформацияОРазработчикеИПрограмме.НаименованиеКонфигурации() Тогда
		// Квартплата -
...
Процедура УстановитьФлагОтображенияОписанийПоТекущуюВерсию(Знач ИмяПользователя = Неопределено) Экспорт
	...
	"ПоследняяВерсияОтображенияИзмененийСистемы", КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия), , ИмяПользователя);
	// Квартплата -
...
Процедура УстановитьВерсиюОписанийИзменений() Экспорт
	...
	ТекущаяВерсия = ВерсияИБ(КВП_ПрочиеПроцедурыИФункции.НаименованиеТиповойКонфигурации(Метаданные.Имя));
	// Квартплата -
...
Функция РазделыОписанияИзменений() Экспорт
	...
	ВесВерсииМетаданных = ВесВерсии(КВП_ПрочиеПроцедурыИФункции.НомерВерсииТиповойКонфигурации(Метаданные.Версия));
	// Квартплата -
