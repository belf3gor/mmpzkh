...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ВыводитьПолностью Тогда
		...
		Попытка
			...
			// Квартплата+
			// Если в параметрах отчета содержатся такие параметры как "ВыводитьДанныеВТаблицу" или "ВыводитьДанныеВДерево"
			// и хотя бы один из них установлен в значение "Истина", то выполнять стандартный код по выводу в табличный документ
			// не нужно, т.к. результат будет выведен далее в таблицу или в дерево.
			ИспользоватьСтандартныйВыводВТабличныйДокумент = НЕ ПараметрыОтчета.Свойство("ВыводитьДанныеВТаблицу") И НЕ ПараметрыОтчета.Свойство("ВыводитьДанныеВДерево");
			Если ИспользоватьСтандартныйВыводВТабличныйДокумент Тогда
			// Квартплата-
			...
			// Квартплата +
			Иначе
				// Пока что используется только в отчете "Сводная ведомость".
				...
				МакетКомпоновкиДляВыводаВКоллекцию = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанных, НастройкиДляКомпоновкиМакета,,, Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
				...
				Если ПараметрыОтчета["ВыводитьДанныеВДерево"] Тогда
					...
					ПроцессорВыводаВКоллекцию.Вывести(ПроцессорКомпоновкиДляВыводаВКоллекцию, Истина);
				КонецЕсли;
				МенеджерОтчета.ВывестиДанныеКоллекцииВДокумент(ПараметрыОтчета, Таблица, Дерево, Результат);
				Если Не Отказ Тогда
