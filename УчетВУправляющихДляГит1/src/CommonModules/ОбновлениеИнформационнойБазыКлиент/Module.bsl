...
Процедура ПередНачаломРаботыСистемы5(Параметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ИнформационнаяБазаФайловая()
	   И СтрНайти(ПараметрЗапуска, "ВыполнитьОбновлениеИЗавершитьРаботу") > 0
	   // Квартплата +
	   ИЛИ СтрНайти(ПараметрЗапуска, "ВыполнитьСценарноеТестирование") > 0 Тогда
	   // Квартплата -
...
Процедура ПоказатьОписаниеИзмененийСистемы()
	...
	Если ПараметрыРаботыКлиента.ПоказатьОписаниеИзмененийСистемы Тогда
		...
		// Квартплата +
		УПЖКХ_ОбновлениеИнформационнойБазыКлиент.ПроверитьНаличиеВнешнихКвитанций();
		УПЖКХ_ОбновлениеИнформационнойБазыКлиент.ПроверитьИспользованиеФормКвитанцийКоторыеПодлежатУдалению(Истина);
		// Квартплата -
