...
Процедура ПриЧтенииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация") Экспорт
	...
	// Добавляем реквизит "ВключитьВСтопЛист" в массив удаляемых реквизитов для того,
	// чтобы при перерисовки формы не выстрелила ошибка: не уникальное имя реквизита.
	
	ИмяВключитьВСтопЛист = УПЖКХ_РассылкаКвитанцийНаЭлектроннуюПочтуЛицевыхСчетовСервер.ПолучитьИмяВключитьВСтопЛист();
	
	Для Каждого ТекРеквизит Из СписокРеквизитовФормы Цикл
		Если ТекРеквизит.Имя = ИмяВключитьВСтопЛист Тогда
			МассивУдаляемыхРеквизитов.Добавить(ИмяВключитьВСтопЛист);
		КонецЕсли;
	КонецЦикла;
...
//
Процедура УдалитьЭлементыКонтактнойИнформацииНаФорме(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация") Экспорт
	...
	ИмяЭлементаДляРазмещения = ПараметрыКИ.ГруппаДляРазмещения;
	
	ПоложениеЗаголовкаКИ = ?(ЗначениеЗаполнено(ПараметрыКИ.ПоложениеЗаголовка), ПредопределенноеЗначение(ПараметрыКИ.ПоложениеЗаголовка), ПоложениеЗаголовкаЭлементаФормы.Лево);
	ВыполненаОтложеннаяИнициализация = ПараметрыКИ.ВыполненаОтложеннаяИнициализация;
	ОтложеннаяИнициализация = ПараметрыКИ.ОтложеннаяИнициализация И Не ВыполненаОтложеннаяИнициализация;
	
	ИспользуетсяКонтактнаяИнформация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаВидовКИ, "Используется");
	Если НЕ ИспользуетсяКонтактнаяИнформация Тогда
		МассивУдаляемыхРеквизитов = ПараметрыКИ.ДобавленныеРеквизиты;
	Иначе
		УдалитьКомандыИЭлементыФормы(Форма, ИмяЭлементаДляРазмещения);
		...
		СтатическиеРеквизиты = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ПараметрыКИ.ИсключаемыеВиды);
		ИменаТабличныхЧастейПоВидамКИ = Неопределено;
	...
	// Производим поиск реквизитов формы с видами КИ, помеченными на удаление. Если вид КИ помечен на удаление в
	// справочнике видов контактной информации, он все равно создается как реквизит формы, но при этом его нет в 
	// таблице с описанием реквизитов КИ формы. Поэтому поиск таких реквизитов и добавление их массив удаляемых
	// реквизитов производится отдельно.
	// Если не удалить оставшиеся реквизиты КИ формы, то при выборе физлица в помощнике ввода проживающего возникает
	// конф. ошибка.
	Для Каждого ЭлементСпискаРеквизитов Из СписокРеквизитовФормы Цикл
		
		Если СтрНайти(ЭлементСпискаРеквизитов.Имя, "КонтактнаяИнформацияПоле", НаправлениеПоиска.СНачала, 1, 1) = 1 Тогда
			
			Если МассивУдаляемыхРеквизитов.Найти(ЭлементСпискаРеквизитов.Имя) = Неопределено Тогда
				МассивУдаляемыхРеквизитов.Добавить(ЭлементСпискаРеквизитов.Имя);
			КонецЕсли;
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь, КоличествоАдресов = Неопределено, СледующаяСтрока = Неопределено)
	...
	// Программное добавление поля "Включить в стоп-лист рассылки квитанций" в справочник "Физические лица". 
	// Данное поле используется для добавления адреса электронной почты физического лица в "Стоп-лист рассылки квитанций".
	
	ТипАдресЭлектроннойПочты = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
	
	ЭтоСтрокаДереваЗначений = (ТипЗнч(Результат) = Тип("СтрокаДереваЗначений"));
	
	Если Форма.ИмяФормы = "Справочник.ФизическиеЛица.Форма.ФормаЭлемента"
		И Форма.Элементы.Найти("ФлагВключитьВСтопЛист") = Неопределено
		И ЭтоСтрокаДереваЗначений И Результат.Тип = ТипАдресЭлектроннойПочты Тогда
		
		НайденныеСтроки = Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Новый Структура("Тип", ТипАдресЭлектроннойПочты));
		Если Не НайденныеСтроки = Неопределено Тогда
			// МеханизмРассылкиКвитанцийНаЭлекторонныйЯщики
			УПЖКХ_РассылкаКвитанцийНаЭлектроннуюПочтуЛицевыхСчетовСервер.СтопЛистПриСозданииНаСервере(Форма, Форма.Элементы["Группа" + ИмяРеквизита], НайденныеСтроки[0].Представление);
			// Конец МеханизмРассылкиКвитанцийНаЭлекторонныйЯщики
		КонецЕсли;
