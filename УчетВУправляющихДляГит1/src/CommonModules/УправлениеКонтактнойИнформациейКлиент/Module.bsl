...
Процедура НачалоВыбора(Форма, Элемент, Модифицированность = Истина, СтандартнаяОбработка = Ложь, ПараметрыОткрытия = Неопределено) Экспорт
	...
	// Квартплата +
	Если Найти(Форма.ИмяФормы, "УПЖКХ_Жильцы") > 0 Тогда
		Если ДанныеСтроки.Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.УПЖКХ_АдресВыбытияЖильца") Тогда
			ПараметрыОткрытияФормы.Вставить("УПЖКХ_Период", Форма.Служебный_ДатаВыбытия);
		ИначеЕсли ДанныеСтроки.Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.УПЖКХ_АдресПрибытияЖильца") Тогда
			ПараметрыОткрытияФормы.Вставить("УПЖКХ_Период", Форма.Служебный_ДатаПрибытия);
		КонецЕсли;
	КонецЕсли;
	// Квартплата -
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	// Квартплата +
	Если РезультатЗакрытия.Свойство("УПЖКХ_Период") Тогда
		Если ДанныеНаФорме.Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.УПЖКХ_АдресВыбытияЖильца") Тогда
			Форма.Служебный_ДатаВыбытия = РезультатЗакрытия.УПЖКХ_Период;
		ИначеЕсли ДанныеНаФорме.Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.УПЖКХ_АдресПрибытияЖильца") Тогда
			Форма.Служебный_ДатаПрибытия = РезультатЗакрытия.УПЖКХ_Период;
		КонецЕсли;
	КонецЕсли;
	// Квартплата -
