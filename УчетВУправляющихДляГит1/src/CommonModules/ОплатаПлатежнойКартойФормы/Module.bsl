...
Процедура ПодготовитьФормуНаСервере(Форма)
	...
	УстановитьСписокВыбораВидаОперации(Форма);
	УстановитьФункциональныеОпцииФормы(Форма);
...
Процедура УстановитьСписокВыбораВидаОперации(Форма)
	...
	УчетБезЗакрывающих = УчетДенежныхСредствБП.УчетБезЗакрывающихДокументовВозможен(Объект.Организация, Объект.Дата);
	
	Если УчетБезЗакрывающих И Объект.Проведен
		И (РозницаВключена И Объект.БезЗакрывающихДокументов
		Или НЕ РозницаВключена И Не Объект.БезЗакрывающихДокументов)
		Или Объект.Проведен И РозницаВключена И Не РозницаДоступна Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементВидОперации = Форма.Элементы.ВидОперации;
	ЭлементВидОперации.СписокВыбора.Очистить();
	ЭлементВидОперации.РежимВыбораИзСписка = Истина;
	ЭлементВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя);
	
	Если РозницаДоступна И (РозницаВключена Или УчетБезЗакрывающих) Тогда
		ЭлементВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийОплатаПлатежнойКартой.РозничнаяВыручка);
	КонецЕсли;
	ЭлементВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийОплатаПлатежнойКартой.ВозвратПокупателю);
	
	Если РозницаДоступна И Не РозницаВключена И Не УчетБезЗакрывающих
		И ОплатаПлатежнойКартойКлиентСервер.ЭтоРозничнаяВыручка(Объект.ВидОперации) Тогда
