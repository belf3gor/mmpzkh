...
Процедура ОтправитьСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки) Экспорт
	...
	// квартплата +
	Если ПараметрыОтправки.Свойство("ЭтоМассоваяРассылка") И ПараметрыОтправки.ЭтоМассоваяРассылка Тогда
		Письмо.УстановитьПолеЗаголовка("Precedence", "bulk", СпособКодированияНеASCIIСимволовИнтернетПочтовогоСообщения.БезКодирования);
	КонецЕсли;
	// квартплата -
