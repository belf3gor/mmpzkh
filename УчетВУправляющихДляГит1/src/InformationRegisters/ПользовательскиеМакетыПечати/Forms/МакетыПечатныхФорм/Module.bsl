...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОТР_ПодключаемыеОбщиеМеханизмыИКоманды.ПриСозданииНаСервере(ЭтаФорма, Элементы.МакетыПечатныхФормКоманднаяПанель);
...
Процедура ПереключитьИспользованиеВыбранныхМакетов(ИспользуетсяИзмененный)
	...
	// Квартплата +
	ВызватьОповещениеОбИзмененииМакета(ПереключаемыеМакеты, ИспользуетсяИзмененный);
	// Квартплата -
...
Процедура УдалитьВыбранныеИзмененныеМакеты(Команда)
	...
	// Квартплата +
	ВызватьОповещениеОбИзмененииМакета(УдаляемыеМакеты, Ложь);
	// Квартплата -
...
Процедура УстановитьДоступностьКнопокКоманднойПанели()
	...
	// Квартплата +
	СравнитьСтандартныйИИзмененныйМакетыДоступность = Ложь;
	ТекущийМакетЯвляетсяТабличнымДокументом         = ТекущийМакетВыбран И ТекущийМакет.ТипМакета = "MXL";
	// Квартплата -
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.МакетыПечатныхФорм.ВыделенныеСтроки Цикл
		...
		// Квартплата +
		СравнитьСтандартныйИИзмененныйМакетыДоступность = ТекущийМакетЯвляетсяТабличнымДокументом И (ТекущийМакетВыбран И ТекущийМакет.Изменен Или ВыбраноНесколькоМакетов И ИспользоватьИзмененныйМакетДоступность ИЛИ ИспользоватьСтандартныйМакетДоступность);
		// Квартплата -
	...
	// Квартплата +
	Элементы.МакетыПечатныхФормСравнитьСТиповымМакетомУПЖКХ.Доступность = СравнитьСтандартныйИИзмененныйМакетыДоступность;
	// Квартплата -
...
// Квартплата +
&НаКлиенте
Процедура ВызватьОповещениеОбИзмененииМакета(Макеты, ИспользуетсяИзмененный)
	
	Для Каждого ТекущийМакет Из Макеты Цикл
		ПараметрыОповещения = Новый Структура("ИмяОбъектаМетаданныхМакета, ИспользуетсяИзмененный", ТекущийМакет, ИспользуетсяИзмененный);
		Оповестить("Запись_ТабличныйДокумент", ПараметрыОповещения, ЭтотОбъект);
	КонецЦикла;
...
#Область МеханизмСравненияТабличныхДокументов

&НаКлиенте
// Обработчик команды "СравнитьСтандартныйИИзмененныйМакеты".
//
Процедура СравнитьСтандартныйИИзмененныйМакеты(Команда)
	
	Для Каждого ВыделеннаяСтрока Из Элементы.МакетыПечатныхФорм.ВыделенныеСтроки Цикл
		ТекущиеДанные = Элементы.МакетыПечатныхФорм.ДанныеСтроки(ВыделеннаяСтрока);
		Если ТекущиеДанные.ТипМакета = "MXL" И ТекущиеДанные.Изменен Тогда
			СтруктураДанныхМакета = ПолучитьСтруктуруДанныхМакета(ТекущиеДанные);
			УПЖКХ_РаботаСДиалогамиКлиент.СравнитьТиповойМакетСПользовательским(СтруктураДанныхМакета.ИмяВладельца, СтруктураДанныхМакета.ИмяМакета, СтруктураДанныхМакета.ПредставлениеМакета, Истина);
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
// Получает структуру данных с подробной информацией о макете.
//
Функция ПолучитьСтруктуруДанныхМакета(ДанныеТаблицыМакетовПечатныхФорм)
	
	ИмяОбъектаМетаданныхМакета = ДанныеТаблицыМакетовПечатныхФорм.ИмяОбъектаМетаданныхМакета;
	ПредставлениеМакета        = ДанныеТаблицыМакетовПечатныхФорм.ПредставлениеМакета;
	...
	Возврат Новый Структура("ИмяВладельца, ИмяМакета, ПредставлениеМакета",
							 ИмяВладельца, ИмяМакета, ПредставлениеМакета);
